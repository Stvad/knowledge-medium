...
- **`typesProp`** ([src/data/properties.ts](src/data/properties.ts)) — a string-list prop named `types`. Set today to `'extension'`, `'page'`, `'panel'`, and `'daily-note'`. The deterministic Journal block is just a page; it no longer has a separate `journal` type.
...
private async _addTypeInTx(
  ...
  for (const [name, value] of Object.entries(initialValues)) {
    if (next[name] === undefined) {
      ...
      if (!schema) {
        throw new Error(
          `[addType] initialValues['${name}'] has no registered PropertySchema in the merged registry. ` +
          `Register the schema via propertySchemasFacet.of(...) or via TypeContribution.properties so it appears in the merged schema registry (§1a), ` +
...
- **Codec.** A property has one codec globally — the **merged `repo.propertySchemas` map** (§1a — populated from both `propertySchemasFacet.of(...)` and type contributions' `properties[]`) is the global codec registry, keyed by `name` and last-wins on duplicates (with type-lifted-first / direct-second pass order — see §1a). Multi-type doesn't change that. If two types want truly incompatible codecs, namespace one of them per the §3 hybrid rule (use a plugin-private `myplugin:status` rather than overloading the shared `status`). Consumers (property panel, typed-query primitive, ref projector via `ctx.propertySchemas`) read the merged map; reading `propertySchemasFacet` directly would miss type-lifted-only schemas.
...
7. Add `KERNEL_TYPE_CONTRIBUTIONS` for `'page'`, `'panel'`, `'daily-note'`, `'extension'`.
