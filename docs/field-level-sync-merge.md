...
- `parseReferences` runs via its existing entry point (`PARSE_REFERENCES_PROCESSOR.apply`), opens its own `repo.tx` for the write phase as today. Idempotency: `planNeedsWrite` returns false when the recomputed refs equal what's already stored, so non-merge-induced property changes (or already-converged states) are cheap no-ops.
