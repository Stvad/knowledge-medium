...
**The backfill emitter (`BACKFILL_LOCAL_EPHEMERAL_UPLOADS_SQL`, `clientSchema.ts:913`) must NOT add `old`.** It intentionally emits the full local row as canonical state ("local wins" recovery — see v4 backfill rationale at `clientSchema.ts:844-868`). Adding `old.properties_json = b.properties_json` would make `new == old`, the uploader's diff would be `{set:{}, unset:[]}`, and the RPC would skip the property update — leaving the server's stale row intact and reintroducing exactly the failure mode v4 was designed to fix. Keep the backfill envelope at `{op, type, id, data}` with `data` being the full row; the uploader interprets "no `old.properties_json` but `data.properties_json` present" as overwrite-the-column, which is the right recovery semantic.
...
**Two intended paths through the uploader**, distinguished by the envelope shape:

- **Per-key merge path:** `data.properties_json` present, `old.properties_json` present (trigger-fired PATCH with a real property change). Diff to `{set, unset}`, ship to RPC, server does shallow JSONB merge.
- **Overwrite path:** `data.properties_json` present, `old.properties_json` absent (backfill PATCH, or any future "I want a full overwrite" caller). Ship `data.properties_json` verbatim to RPC, server does `properties_json = $value`. Same semantic as today's full-row UPSERT, scoped to the property column.

Both are first-class, not "fallback for unexpected shape." A trigger-fired PATCH that didn't change `properties_json` has neither field set, and the RPC simply doesn't touch the column.
...
- **Backfill PATCH (`BACKFILL_LOCAL_EPHEMERAL_UPLOADS_SQL`).** Emits `{op, type, id, data}` with the full local row and *no* `old` field. Uploader takes the overwrite path: `properties_json` is replaced verbatim, content/references are replaced verbatim. This is the v4 backfill's recovery semantic ("local wins for these rows") and must be preserved. Adding `old` to the backfill envelope would silently break recovery — diff is empty, RPC skips the column, server's stale row sticks. The doc/code should make this distinction explicit (e.g., a comment in the backfill SQL and in the RPC) so a future "make backfill and trigger envelopes match" refactor doesn't reintroduce the bug.
