...
- **Derived `references_json` is briefly inconsistent after concurrent ref-property edits.** `parseReferences` (post-commit processor in `src/plugins/references/`) watches property changes and writes the derived `references_json` column via its own `tx.update`. Each client's `references_json` write reflects only that client's local properties at the time. After Layer B merges properties_json across clients but leaves `references_json` as column-LWW, the server's references_json briefly shows only the last writer's view — backlinks/typed queries for refs that didn't win the column race are temporarily missing. **Self-healing:** when each client receives the synced merged properties, `parseReferences` re-runs locally and uploads a correctly merged `references_json`. Window is sync-latency + parseReferences debounce, typically seconds. Strictly better than today (today both columns get clobbered, not just one), but worth flagging. A more durable fix is server-side: recompute `references_json` inside the RPC after the properties_json merge — requires porting `parseReferences` to SQL/PLpgSQL. Out of scope here; revisit if telemetry shows the inconsistency window mattering in practice.
