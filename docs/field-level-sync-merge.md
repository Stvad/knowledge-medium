...
```sql
UPDATE ps_crud
SET data = json_set(data, '$.metadata', json_object('mode', 'upsert'))
...
- **Schema migration.** Three parts. **Client-side trigger:** `CLIENT_SCHEMA_STATEMENTS` runs `CREATE TRIGGER IF NOT EXISTS` (`repoProvider.ts:237-239`), so upgrading clients keep the old `blocks_upload_update` definition unless we explicitly drop it. Plan adds `DROP TRIGGER IF EXISTS blocks_upload_update` before the CREATE in `CLIENT_SCHEMA_STATEMENTS`. Safe to run every bootstrap (DROP is O(1) and idempotent). **Client-side legacy queue:** existing `ps_crud` entries from the old trigger have `data.properties_json` set but no `old` or `metadata.mode`. A one-shot `client_schema_state`-gated SQL UPDATE stamps them with `metadata.mode = 'upsert'` so they take the explicit upsert path (same blast radius as today, not worse). Runs before the trigger DROP+CREATE so the queue is consistent before any new code reads it. **Server-side:** the migration adds the RPC function only — no table changes, no column-type changes. Old clients keep working unchanged (they go through PostgREST `.update()` against the same `text` columns).
