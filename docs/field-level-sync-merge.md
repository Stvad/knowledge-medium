...
Written 2026-05-27. Scope: `src/data/internals/clientSchema.ts` (custom upload trigger; also drops v4 backfill machinery as a phase-0 cleanup), `src/services/powersync.ts` (uploader), `src/data/internals/txEngine.ts` (semantic-no-op guard), one Supabase migration adding an RPC. No schema reset; no on-disk data migration; no Postgres column-type changes. **Rollout assumption: all clients upgrade in one go (no long-lived mixed-version state).** **Cleanup assumption: the v4 local-ephemeral backfill (`clientSchema.ts:822-935`) has run on every active client; it gets deleted in phase 0 of this sequence before any other change lands.**
...
- **Delete the v4 local-ephemeral backfill as phase 0.** `BACKFILL_LOCAL_EPHEMERAL_UPLOADS_SQL`, `LOCAL_EPHEMERAL_BACKFILL_MARKER_KEY`, `COUNT_LOCAL_EPHEMERAL_BACKFILL_PENDING_SQL`, and their bootstrap call sites (`clientSchema.ts:822-935`) come out in their own commit/PR ahead of the merge work. Doing this first keeps every subsequent layer from having to plan for a second "full-row PATCH" emitter shape, and keeps the LWW PRs focused on the merge fix. **Assumption being made explicit:** the per-client v4 marker on every *active* client has fired, but a truly dormant client that hasn't been online since pre-v4 would upgrade past v4 without recovery — its never-uploaded local-ephemeral UI-state rows wouldn't be backfilled. Accepted trade-off for alpha; revisit if such a client is ever observed.
...
`compactBlockCrudEntries` (`powersync.ts:112`) keeps fusing same-tx PUT+PATCH and PATCH+PATCH **only when shape-compatible**. **`CompactedBlockOperation` (`powersync.ts:25-42`) currently carries only `{kind, id, payload, order}` — extend with `previousValues?: {properties_json?: string}`** so the trigger's `old.properties_json` survives into the uploader. Two fusion rules:

- **Don't fuse across the legacy/new boundary.** If one PATCH for an id is legacy-shape (no `data.old` field at envelope level → `entry.previousValues === undefined` after `CrudEntry.fromRow`) and another is new-shape (`entry.previousValues` defined), keep them as separate operations in the compacted output. The legacy one ships through the fallback (full-row UPSERT), the new one ships through the RPC. Otherwise the legacy intent is silently dropped: fusing carries the new PATCH's `previousValues` as the diff base, and any keys the legacy PATCH carried (but the new PATCH didn't change) get omitted from the server-side merge.
- **Within an all-new-shape chain, fusion is per-column.** For each tracked column, take the value from the earliest PATCH in the chain that *actually carried that column in `old`*. With only `properties_json` tracked today that simplifies to "first new-shape PATCH with `previousValues.properties_json` defined wins" — distinct from "first PATCH wins outright." Concrete reason: same-tx writes can interleave column edits (e.g., `tx.update({content})` then `tx.setProperty(foo, …)`); the first PATCH's `old` is `{}` because it didn't touch properties_json, the second's `old.properties_json` is the true prior. Taking the first PATCH's `{}` would make the fused entry's `opData.properties_json` present with no matching `previousValues.properties_json`, which contradicts the invariant below and would either quarantine a valid tx or skip the merge.
...
- **Mixed legacy + new compaction.** Critical: the compactor must *not* fuse a legacy PATCH with a new-shape PATCH for the same id within the same batch. Scenario: pre-upgrade legacy PATCH for block X carries some pending property keys; user makes a new property edit on X after upgrade; new trigger emits a new-shape PATCH with `old.properties_json` reflecting the local state *after* the legacy PATCH was applied locally. Fusing would compute a diff from the new PATCH's `old`, silently dropping the legacy PATCH's contribution to keys the new PATCH didn't change. Rule: if any participant in a same-id PATCH fusion lacks `entry.previousValues`, keep all participants split. Legacy ships first via fallback (preserves its full-row intent), new ships separately via RPC.
...
- **Derived `references_json` is briefly inconsistent after concurrent ref-property edits.** `parseReferences` (post-commit processor in `src/plugins/references/`) watches property changes and writes the derived `references_json` column via its own `tx.update`. Each client's `references_json` write reflects only that client's local properties at the time. After Layer B merges properties_json across clients but leaves `references_json` as column-LWW, the server's references_json briefly shows only the last writer's view — backlinks/typed queries for refs that didn't win the column race are temporarily missing. **Self-healing is only partial:** `rowEventsTail` (`src/data/internals/rowEventsTail.ts`) updates cache and invalidates handles on sync arrival but does *not* dispatch post-commit processors — `parseReferences` is dispatched from `repo.tx` outcomes only. So the stale `references_json` persists until the next *local* write to that block (which fires the field watcher with the now-merged properties and a recompute). For frequently-edited blocks this is seconds; for rarely-touched blocks the window can be long. Still strictly better than today (today *both* props and refs get clobbered; new behavior only loses the derived refs, and only until the next local edit on that block). A durable fix is server-side recompute inside the RPC after the property merge, or extending `rowEventsTail` to dispatch processors on sync-arrived field changes. Both out of scope here; revisit if telemetry shows the inconsistency window mattering in practice.
