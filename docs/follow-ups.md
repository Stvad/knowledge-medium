...
## Restart Electric shapes after workspace membership changes

The first Electric deployment can tolerate "reload after join" for workspace access changes, but production should not leave that as an implicit rule. The migrated client currently opens one Electric shape stream per synced table per user, and the Supabase Edge proxy scopes each shape from the workspace IDs visible to the bearer token when that stream starts. If a user creates a workspace, accepts an invite, or otherwise gains membership while the app is already open, existing shape streams may keep their old workspace predicate until they are restarted.

Fix shape: after the create/accept workspace membership flows commit remotely, restart the Electric shape subscriber for the signed-in user so `/blocks`, `/workspaces`, and `/workspace_members` reconnect with the expanded workspace set. A more robust later version could move to a proxy/client shape strategy that tracks dynamic membership changes without a full subscriber restart. Until one of those lands, deployment notes and product copy should treat reload-after-join as the known V1 behavior. This is a visibility freshness issue, not a local data-loss path: a reload or subscriber restart reopens shapes and receives the newly authorized snapshot.
