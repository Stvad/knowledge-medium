...
Audited 2026-05-26 on `ff-vlad-dev`: 424 tombstoned alias seats match the exact restorability predicate the probe now uses (pristine seed shape, `content === alias[0]`, no live children, no inbound `block_references`). Worst-offender prefix: `browser em` at 20 tombstones — accumulated en route to typing `[[browser emacs]]`. The full table:
