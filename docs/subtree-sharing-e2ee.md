...
> **Status:** unverified (design exploration, 2026-06-24; media status restamped 2026-07-01). E2EE seams, RLS, and grants verified against code (§11); §13-hierarchy facts are design-only; media facts come from the media design doc, which has **since shipped to master** (phases 1–6, PRs #265/#269/#273) — see §11. This is a *new* doc; it does **not** replace [`subtree-sharing.md`](subtree-sharing.md), which remains the canonical write-up of the **access-control plane** for a non-encrypted world. This doc layers the **key-access plane** on top and revises the parts of the old design that assumed a plaintext-reading server. Where the two disagree, prefer this doc for anything touching encryption and the old doc for the RLS / `effective_share_ids` mechanics it still owns.
...
- **C-in-place (multiple key domains per workspace).** The subtree's blocks stay in the origin workspace but are re-sealed under `SK`. This generalizes the e2ee core from *one key per workspace* to *a key domain per (workspace, key-id)*, and the ripple is wider than just the seam:
  ...
  - The **mode pin** ([`modePin.ts`](../src/sync/keys/modePin.ts)) is per `(user, workspace)` and set-once/immutable — it has no slot for a sub-workspace domain, so "is this block's domain e2ee and is its SK loaded?" needs new sub-workspace authority machinery, exactly the authority the §6/§8.3 gate is built on. The same workspace-shape holds for the redemption path: the `wk_canary` and the `(user, workspace)`-keyed key store have no share-scoped analogs, so *validating and storing* a delivered `SK` is new machinery too (§6.F).
...
An asset lives at `<workspace_id>/HMAC(K_id, sha256(plaintext))`, sealed `encb:v1:` under the WK, `K_id = HKDF(WK, …)` (media now shipped on master; facts from its design doc — §11).
...
5. **Reprojection schema-parity gate** (§8.2): how do we confirm an editor recipient's client loads the owner's ref-deriving schemas? Crypto-agnostic — gates the *shipping* plaintext Option A editor path too.
...
| Media content path/seal | `HMAC(K_id,…)`, `encb:v1:`, media design (shipped on master) | B/E: per-workspace Storage RLS; subtree-granular needs `content_key → block`; C: re-key/re-upload (§8.1) |
...
- **Media — shipped after this doc's verification pass (restamped 2026-07-01):** media facts (`HMAC(K_id)` path, `K_id = HKDF(WK,…)`, `encb:v1:`, per-workspace Storage RLS, dedup, export-to-plaintext, deferred `content_key → block` mapping) are from `docs/media-attachments/design.html`, written when it was design-only. Media has since **merged to master** (phases 1–6: PRs #265/#269/#273, 2026-06-27→29; `supabase/migrations/20260625000000_add_attachments_storage.sql`), and the design doc now lives on master too. These facts were NOT re-verified against the shipped code from this branch — spot-check at implementation time.
- **Still design-only:** e2ee §13-hierarchy facts (per-workspace WK, no asymmetric crypto in v1, deferred passphrase/UVK/X25519 hierarchy, "no server-side features for e2ee," out-of-band key transfer) are from `docs/e2ee-design.html`, whose status banner reads "superseded in places" — load-bearing claims here are pinned to code, not that doc; no vault/X25519 code exists in `src/sync/keys/`.
...
- `docs/media-attachments/design.html` (on master; shipped via PRs #265/#269/#273) — encrypted byte storage, content-addressing, and its own deferral of encrypted subtree sharing (deferral *rationale* superseded — §1.2, §6.F).
