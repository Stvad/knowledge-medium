...
- **Recommendation (§7): a phased hybrid that does not over-promise.** Ship the crypto-agnostic access-control plane so it fully works on **plaintext** workspaces (the old design). For **e2ee** workspaces, v1 offers only honest, low-crypto-risk paths: **(D)** *export-to-plaintext* for read-only/public/anonymous links (a deliberate declassification), and an honestly-labelled **add-collaborator** affordance — either the existing whole-workspace share or a **manual second e2ee workspace (E)** for confidential-but-trusted cases. We explicitly **do not** ship confidential subtree sharing on e2ee (Option C) in v1 — that is the marquee capability and the central product bet (§7) — because it needs a new key domain or a re-encryption relay plus the deferred e2ee §13 hierarchy. There is no anonymous link that keeps bytes encrypted until C ships; the only v1 anonymous e2ee "share" is D, which gives up encryption for that copy.
...
- **§3.3 / §8.4 read-only snapshot RPC `get_shared_subtree(token)`** returns `{ blocks: [BlockData…] }` server-assembled. On e2ee the content columns are `enc:v1:` ciphertext, so a `SECURITY DEFINER` function returns only ciphertext. It still *runs*, but it is **useless to a keyless recipient** — so it must be disabled or rerouted for e2ee, not relied on. Anonymous read-only e2ee sharing cannot go through it (replacement: §6.D export, or §6.C key-in-fragment).
...
**Verdict on B's existence.** Because B is cryptographically "hand over the master key," it is *barely distinguishable from adding a collaborator to the workspace* with a focused landing block — and Option E (below) gives a trusted collaborator strictly better confidentiality at comparable user effort. The recommendation (§7) therefore does **not** ship B as a distinct "subtree share" feature; it surfaces the underlying mechanism only as an explicitly-labelled add-collaborator/whole-workspace-share affordance. B is retained here as a named point in the option space precisely so the reasoning against shipping it is on the record.
...
- **C-in-place (multiple key domains per workspace).** The subtree's blocks stay in the origin workspace but are re-sealed under `SK`. This generalizes the e2ee core from *one key per workspace* to *a key domain per (workspace, key-id)*, and the ripple is wider than just the seam:
  - `getCek` must resolve per (workspace, key-id), not per workspace.
  - The per-column AAD must additionally bind a **`key_id`**. This is *not cosmetic*: today the AAD pins `[block_id, workspace_id, column, schema_version]` but **not** which key domain sealed the column. With WK- and SK-sealed ciphertext sharing the same `(block_id, workspace_id, column)` AAD, nothing authenticates the domain, enabling a cross-domain splice or wrong-domain-decrypt by a both-keys holder or a dishonest server. Binding `key_id` authenticates the domain (and bumps `schema_version`).
...
- **Reprojection write-back (the dangerous one).** `references_json` is a *client-computed, encrypted* derived column ([`src/data/api/derivedData.ts`](../src/data/api/derivedData.ts)). An **editor recipient runs the client reprojection engine over a *partial* workspace view** — only the shared subtree is synced to them. This is exactly the configuration behind the SRS / daily-note ref-stripping incidents, where a client without full context reprojected and *stripped* derived refs. A recipient's edit must **not** be able to strip `references_json` on blocks whose ref-sources lie outside the recipient's sync view. The standing principle (derived refs tied to a field's *value*, reprojection **add-only / never strips on absence**) must be proven to hold for the partial-view recipient; otherwise this design silently corrupts owner data. This is the single most likely correctness failure in the whole feature and is a hard gate on any editor-share path (B, C, or even E with a sanctioned move). Backlinks pointing *into* a shared subtree from outside are computed on the owner's full-context client and are simply invisible to the recipient — acceptable, but state it.
...
- **Key loss.** Recipient loses their device key → for B, re-paste the WK; for C/E, the owner re-shares the `SK`/second-workspace WK. There is no recovery of lost keys (inherited e2ee property).
...
- **Option C (per-share SK) / E (second workspace):** revocation is **meaningful and cheap** — rotate `SK` (C) / the second-workspace WK (E), re-seal only the (small) shared scope, re-wrap to remaining members. The recipient keeps what they already pulled but is cryptographically locked out of everything after rotation.
...
- **B:** the invariant now rests on **(i) the recipient being a share-member-only, (ii) an honest server enforcing the union RLS, and (iii) no direct-PostgREST escape** — and even then on the recipient's trustworthiness for the whole workspace, since they hold the master key. A deliberate, scoped relaxation; acceptable for fully-trusted collaborators, unacceptable as a general primitive. (Same enumeration as the §3.1 table — keep them in sync.)
...
5. **Reprojection safety for partial-view editors** (§8.2): prove the add-only/never-strip-on-absence property holds for a recipient who syncs only the subtree, before any editor-share path ships. Hard gate.
...
| Whole-workspace e2ee share | `createEncryptedWorkspace` + `unlockWorkspaceWithKey` + workspace-scoped canary, [`shareWorkspace.test.ts`](../src/sync/keys/flows/shareWorkspace.test.ts) | the mechanism behind E and the e2ee "add collaborator" affordance (no `shareWorkspace()` function exists) |
...
| Derived `references_json` | client reprojection, [`derivedData.ts`](../src/data/api/derivedData.ts) | partial-view editor recipients must not strip refs (§8.2) — hard gate |
