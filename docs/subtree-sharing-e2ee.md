...
- **Key loss.** Recipient loses their device key → for B, re-paste the WK; for C/E, the owner re-shares the `SK`/second-workspace WK. There is no recovery of lost keys (inherited e2ee property). Lock-and-wipe is **device-global** (it wipes all local keys, not per-workspace), so a recipient who locks-and-wipes simply re-enters via this same key-loss path; no share-specific recovery is needed.
