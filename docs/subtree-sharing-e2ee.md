...
- **Recommendation (§7): a phased hybrid that does not over-promise.** Ship the crypto-agnostic access-control plane so it fully works on **plaintext** workspaces (the old design). For **e2ee** workspaces, v1 offers only honest, low-crypto-risk paths: **(D)** *export-to-plaintext* for read-only/public/anonymous links (a deliberate declassification), and an **add-collaborator** affordance that is simply the *already-shipped whole-workspace share*, with a **manual second e2ee workspace (E)** as the clean path when the collaborator should be confined to a confidential subtree. (Option B's construction — whole WK + RLS-scoped subtree — is documented below but shipped in **no** form, because it implies a confidentiality wall that does not exist.) We explicitly **do not** ship confidential subtree sharing on e2ee (Option C) in v1 — that is the marquee capability and the central product bet (§7) — because it needs a new key domain or a re-encryption relay: an encryption-core project. (Its key *delivery* can be out of band, exactly like the shipped whole-workspace share — C does **not** hinge on the deferred e2ee §13 hierarchy; §6's delivery subsection.) There is no anonymous link that keeps bytes encrypted until C ships; the only v1 anonymous e2ee "share" is D, which gives up encryption for that copy.
...
### The delivery upgrade (Axis 2) — a convenience, decoupled from C

The e2ee §13 hierarchy (per-user asymmetric keypair — X25519 *as currently sketched* in that doc, which is "superseded in places" — per-workspace CEK wrapped per member, server-brokered delivery) removes the out-of-band step from **B/C/E** without changing their key-domain semantics: the owner wraps the relevant key (WK or SK) to the recipient's published public key; the server stores/relays the wrapped blob; the recipient unwraps locally. It generalizes cleanly to per-share keys — "a CEK wrapped per member, where the CEK belongs to a *workspace OR a share*" — and *that* is the precise sense in which a key hierarchy "helps share subtrees with their own key": it is the **delivery and revocation substrate**, not the thing that mints the per-share key. Minting the per-share key is C's Axis-1 change; the hierarchy only changes how that key reaches a recipient.

**C does not depend on §13.** A per-share `SK` can be delivered exactly the way the shipped whole-workspace share delivers a WK — out of band — and a confidential subtree share is a *deliberate, low-N act* (like sharing a whole workspace), so hand-delivering a key per *share* is no worse here than the per-workspace out-of-band step the design already accepts. Anonymous C links use key-in-fragment, which needs no identity keys at all. So §13 is an Axis-2 *convenience* layered on B/C/E, never a prerequisite; C's real cost is its Axis-1 encryption-core change (§6.C), which §13 does nothing to reduce.

**Subtree sharing therefore does *not* specially justify building §13.** The tempting intuition — "you'd be hand-delivering a key per shared *page*, so sharing forces the hierarchy" — imports a high-frequency, link-per-paragraph sharing model this product doesn't have; a *subtree* is a deliberate unit, not a paragraph. Out-of-band delivery cost scales with *(distinct shares × recipients-per-share × rotations)*; at ≈2 users and a handful of deliberate shares all three factors are tiny. §13 stands on its **own** merits — multi-device unlock without re-pasting the key per workspace, and passphrase-rooted recovery — which sharing neither creates nor removes. It begins to pay off *for sharing* only if real usage trends toward many recipients per share or frequent rotation, where out-of-band redelivery (per-recipient, per-rotation) compounds and automatic re-wrapping wins; revisit then. Until then, keep the key-delivery call site on the existing `getCek`/key-store seam so a future §13 is a delivery swap, not a redesign.
...
- Build only when there is a real need for "share a page, not my graph" *on encrypted workspaces with live collaboration*. Its cost is the Axis-1 encryption-core change, **not** delivery: a per-share `SK` can ship out of band like the existing whole-workspace share, so C does **not** wait on the §13 hierarchy (which stays an optional Axis-2 convenience justified on multi-device/recovery — see §6's delivery subsection). Prefer **C-in-place** only if its encryption-core change (key-id AAD, per-domain `getCek`, sub-workspace pin authority, per-domain materializability) proves cheaper than **C-projection**'s owner-relay reconciliation; prototype both against the media-dedup and offline-collaboration constraints before committing. Note metadata leakage (§8.6) still isn't closed even by C.
...
- **Option C (per-share SK):** revocation is **meaningful and cheap** — rotate `SK`, re-seal only the (small) shared scope, and re-deliver the new `SK` to remaining members (one out-of-band step each, or an automatic re-wrap if §13 exists). The recipient keeps what they already pulled but is cryptographically locked out of everything after rotation. The cheap part is the scoped re-seal; redelivery cost scales with the remaining audience, which is the one place §13's automatic re-wrapping starts to earn its keep (§6 delivery subsection).
