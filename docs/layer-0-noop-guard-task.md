...
- **Do NOT pin the workspace on the no-op path.** `pinWorkspace` is currently called *after* the SQL UPDATE succeeds, and that's the right ordering — keep it that way. The pin isn't just an internal consistency flag: at tx finalize, `command_events.workspace_id` is copied from the pin, and afterCommit dispatch for workspace-scoped processors keys off it. Pinning on a no-op would emit a `command_events` row with `workspace_id` populated (falsely signalling "user did something in workspace X" when nothing happened) and would fire workspace-scoped afterCommit processors for a write that produced no row_events. The correct semantic for a Layer 0–suppressed call is "nothing happened" — no row_events, no `ps_crud`, no `command_events.workspace_id`, no afterCommit dispatch. Leave `pinWorkspace` after the SQL UPDATE; the early return path simply doesn't reach it. The first *real* write in the tx still pins via its own path; subsequent no-ops in the same tx leave the existing pin untouched (which is correct, since other writes already established the workspace).
- **Keep `checkWorkspace(before.workspaceId)` running on the no-op path.** It's a cheap assertion that this tx isn't reaching into the wrong workspace, and the check is meaningful regardless of whether the write turns out to be a no-op — you can't ask "is this a no-op?" in a workspace you have no business touching in the first place. Order of operations: load `before`, run `checkWorkspace`, build proposed `after`, compare, early-return on match; otherwise fall through to metadataPatch + SQL + `pinWorkspace` + `recordWrite`.
...
Plus one cross-cutting test: a tx whose *only* operation is a no-op (e.g. `tx.setProperty(id, schemaName, currentValue)`) finalizes with `command_events.workspace_id` unset and does not trigger workspace-scoped afterCommit dispatch. This pins the load-bearing pin-on-no-op behavior — easy to silently regress by reordering `pinWorkspace` past the guard.
