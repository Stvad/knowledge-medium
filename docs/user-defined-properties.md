...
4. **Build schema objects up-front, through the same validation boundary as `addSchema`.** For each newly-classified name, run `preset.defaultConfig ?? {}` through `preset.configCodec.decode` to get a parsed config (using `defaultConfig` directly without the codec round-trip would skip normalization that the subscription-rebuilt schema goes through, producing two structurally-different schemas for the same `presetId` until the subscription tick). Build the `PropertySchema` from the parsed config; persist `preset.configCodec.encode(parsed)` into the property-schema block (step 5) so the round-trip is canonical. The plan now carries `{schemaBlock: BlockData, schema: PropertySchema}` pairs — block plus runtime registration object, decided at plan time, equivalent to what `addSchema` would produce for the same args.
