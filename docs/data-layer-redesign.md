...
- `repo.query.X(args)` accessor surface (typed via module augmentation) and `repo.runQuery('name', args)`. A single global query-registry epoch folded into the dispatcher's handle-store key, bumped by `swapQueries` whenever the registry changes (any query instance replaced / added / removed). A swap re-keys every query, so a fresh lookup binds to the new registry while live handles keep serving their captured snapshot (immutable-handle rule). A per-name scheme was tried but under-invalidates *composed* queries (`ctx.run`) whose helper set wasn't observed before the swap; see the `queryEpoch` field doc on `Repo`.
