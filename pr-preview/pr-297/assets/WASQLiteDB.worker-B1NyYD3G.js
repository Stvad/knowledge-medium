...
async function asyncModuleFactory(encryptionKey) {
	if (encryptionKey) {
		const { default: factory } = await import("./mc-wa-sqlite-async-D6YA1JmR.js");
		return factory();
	} else {
		const { default: factory } = await import("./wa-sqlite-async-WSRaQNDN.js");
		return factory();
	}
}
async function syncModuleFactory(encryptionKey) {
	if (encryptionKey) {
		const { default: factory } = await import("./mc-wa-sqlite-CNneuMw_.js");
		return factory();
	} else {
		const { default: factory } = await import("./wa-sqlite-8K8WV0lf.js");
		return factory();
	}
}
/**
...
const DEFAULT_MODULE_FACTORIES = {
	[WASQLiteVFS.IDBBatchAtomicVFS]: async (options) => {
		...
		const { IDBBatchAtomicVFS } = await import("./IDBBatchAtomicVFS-CFxOoLWU.js");
		return {
			module,
	...
	[WASQLiteVFS.AccessHandlePoolVFS]: async (options) => {
		...
		const { AccessHandlePoolVFS } = await import("./AccessHandlePoolVFS-CbrTQpok.js");
		return {
			module,
	...
	[WASQLiteVFS.OPFSCoopSyncVFS]: async (options) => {
		const module = await syncModuleFactory(options.encryptionKey);
		const { OPFSCoopSyncVFS } = await import("./OPFSCoopSyncVFS-DZ590PgZ.js");
		return {
			module,
	...
	[WASQLiteVFS.OPFSWriteAheadVFS]: async (options) => {
		const module = await syncModuleFactory(options.encryptionKey);
		const { OPFSWriteAheadVFS } = await import("./OPFSWriteAheadVFS-D_xwx4VP.js");
		return {
			module,
...
//# sourceMappingURL=WASQLiteDB.worker-B1NyYD3G.js.map