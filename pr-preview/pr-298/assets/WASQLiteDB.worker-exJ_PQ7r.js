...
async function asyncModuleFactory(encryptionKey) {
	if (encryptionKey) {
		const { default: factory } = await import("./mc-wa-sqlite-async-D0PZiPU7.js");
		return factory();
	} else {
		const { default: factory } = await import("./wa-sqlite-async-DY0pO8bA.js");
		return factory();
	}
}
async function syncModuleFactory(encryptionKey) {
	if (encryptionKey) {
		const { default: factory } = await import("./mc-wa-sqlite-BmltUQ9e.js");
		return factory();
	} else {
		const { default: factory } = await import("./wa-sqlite-C4BB8fV8.js");
		return factory();
	}
}
/**
...
const DEFAULT_MODULE_FACTORIES = {
	[WASQLiteVFS.IDBBatchAtomicVFS]: async (options) => {
		...
		const { IDBBatchAtomicVFS } = await import("./IDBBatchAtomicVFS-D1INkpDo.js");
		return {
			module,
	...
	[WASQLiteVFS.AccessHandlePoolVFS]: async (options) => {
		...
		const { AccessHandlePoolVFS } = await import("./AccessHandlePoolVFS-Dn7Oop0S.js");
		return {
			module,
	...
	[WASQLiteVFS.OPFSCoopSyncVFS]: async (options) => {
		const module = await syncModuleFactory(options.encryptionKey);
		const { OPFSCoopSyncVFS } = await import("./OPFSCoopSyncVFS-B4ZoAbHc.js");
		return {
			module,
	...
	[WASQLiteVFS.OPFSWriteAheadVFS]: async (options) => {
		const module = await syncModuleFactory(options.encryptionKey);
		const { OPFSWriteAheadVFS } = await import("./OPFSWriteAheadVFS-DrRtTQaA.js");
		return {
			module,
...
//# sourceMappingURL=WASQLiteDB.worker-exJ_PQ7r.js.map