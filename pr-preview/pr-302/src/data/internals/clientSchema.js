...
`.trim(),T=`CASE
      ...
      END`,E=`COALESCE((SELECT source FROM tx_context WHERE id = 1), 'sync')`,D=`
...
`,be=`SELECT COUNT(*) AS count FROM blocks`,xe=`reproject_ref:`,Se=`
  SELECT key FROM client_schema_state WHERE key LIKE '${xe}%'
`,Ce=`
  INSERT OR REPLACE INTO client_schema_state (key, completed_at)
  VALUES (?, strftime('%s', 'now') * 1000)
`,we=`
  DELETE FROM client_schema_state WHERE key = ?
`,Te=`workspace_backfill:`,Ee=`
  SELECT key FROM client_schema_state WHERE key LIKE '${Te}%'
`,De=`
  INSERT OR REPLACE INTO client_schema_state (key, completed_at)
  VALUES (?, strftime('%s', 'now') * 1000)
`,Oe=`reconcile_rescan_v1:`,ke=`
  SELECT key FROM client_schema_state WHERE key = ?
`,Ae=`
  INSERT OR REPLACE INTO client_schema_state (key, completed_at)
  VALUES (?, strftime('%s', 'now') * 1000)
`,je=/^\s*CREATE\s+TRIGGER\s+(?:IF\s+NOT\s+EXISTS\s+)?([A-Za-z0-9_]+)/i,Me=e=>e.flatMap(e=>{let t=e.match(je)?.[1];return t?[`DROP TRIGGER IF EXISTS ${t}`,e]:[e]}),Ne=async(e,t,n,r)=>{await e.execute(`DROP TRIGGER IF EXISTS ${t}`);try{await r()}finally{await e.execute(n)}},Pe=Me([r,i,a,o,s,c,l,u,d,f,ee,ne,m,h,g,re,te,_,v,y,b,x,p,D,O,k,M,N,P,F,I,L,R,z,B,V,H,U,W,K,q,J,S,C]),Fe=[`blocks_row_event_insert`,`blocks_row_event_update`,`blocks_row_event_delete`,`blocks_upload_insert`,`blocks_upload_update`,`blocks_parent_workspace_check_insert`,`blocks_parent_workspace_check_update`,`blocks_parent_not_deleted_check_insert`,`blocks_parent_not_deleted_check_update`,`blocks_alias_insert`,`blocks_alias_update`,`blocks_alias_delete`,`block_aliases_workspace_alias_unique`,`blocks_type_insert`,`blocks_type_update`,`blocks_type_delete`,`blocks_fts_insert`,`blocks_fts_update`,`blocks_fts_delete`,`blocks_synced_changes_insert`,`blocks_synced_changes_delete`],Ie=async e=>{await e.getOptional(X)===null&&(await e.execute(ce),await e.execute(se))},Le=async e=>{await e.getOptional(le)===null&&(await e.execute(de),await e.execute(ue))},Re=async e=>{await e.getOptional(fe)===null&&(await e.execute(me),await e.execute(he),await e.execute(pe))},ze=async e=>{let t=!1;for(let n of[`blocks`,`blocks_synced`])(await e.getAll(`PRAGMA table_info(${n})`)).some(e=>e.name===`user_updated_at`)||(await e.execute(`ALTER TABLE ${n} ADD COLUMN user_updated_at INTEGER`),n===`blocks`&&(t=!0));t&&await Ne(e,`blocks_row_event_update`,O,async()=>{await e.execute(`UPDATE blocks SET user_updated_at = updated_at`)})},Be=async e=>await e.getOptional(ye)===null?null:(await e.getOptional(ve))?.rows??null,$=async e=>(await e.getOptional(be))?.count??0,Ve=(e,t,n=ge,r=4)=>t<n?!1:e===null||t>=e*r||e>=t*r,He=async(e,t={})=>{let n=t.minBlocks??1e3,r=t.growthFactor??4,i=await Be(e),a=await $(e);return Ve(i,a,n,r)?(await e.execute(`ANALYZE`),{analyzed:!0,count:a,previousEstimate:i}):{analyzed:!1,count:a,previousEstimate:i}},Ue=async e=>{let t=await $(e);return await e.execute(`ANALYZE`),{count:t}};export{Y as ALIAS_BACKFILL_MARKER_KEY,_e as ANALYZE_GROWTH_FACTOR,ge as ANALYZE_MIN_BLOCKS,me as BACKFILL_BLOCKS_FTS_ROWIDS_SQL,he as BACKFILL_BLOCKS_FTS_SQL,ce as BACKFILL_BLOCK_ALIASES_SQL,de as BACKFILL_BLOCK_TYPES_SQL,Q as BLOCKS_FTS_BACKFILL_MARKER_KEY,Z as BLOCK_TYPES_BACKFILL_MARKER_KEY,we as CLEAR_REPROJECT_REF_MARKER_SQL,Pe as CLIENT_SCHEMA_STATEMENTS,Fe as CLIENT_SCHEMA_TRIGGER_NAMES,B as CREATE_BLOCKS_ALIAS_DELETE_TRIGGER_SQL,R as CREATE_BLOCKS_ALIAS_INSERT_TRIGGER_SQL,z as CREATE_BLOCKS_ALIAS_UPDATE_TRIGGER_SQL,k as CREATE_BLOCKS_DELETE_ROW_EVENT_TRIGGER_SQL,J as CREATE_BLOCKS_FTS_DELETE_TRIGGER_SQL,K as CREATE_BLOCKS_FTS_INSERT_TRIGGER_SQL,g as CREATE_BLOCKS_FTS_ROWIDS_TABLE_SQL,re as CREATE_BLOCKS_FTS_TABLE_SQL,q as CREATE_BLOCKS_FTS_UPDATE_TRIGGER_SQL,D as CREATE_BLOCKS_INSERT_ROW_EVENT_TRIGGER_SQL,I as CREATE_BLOCKS_PARENT_NOT_DELETED_INSERT_TRIGGER_SQL,L as CREATE_BLOCKS_PARENT_NOT_DELETED_UPDATE_TRIGGER_SQL,C as CREATE_BLOCKS_SYNCED_CHANGES_DELETE_TRIGGER_SQL,x as CREATE_BLOCKS_SYNCED_CHANGES_ID_OP_INDEX_SQL,S as CREATE_BLOCKS_SYNCED_CHANGES_INSERT_TRIGGER_SQL,b as CREATE_BLOCKS_SYNCED_CHANGES_TABLE_SQL,W as CREATE_BLOCKS_TYPE_DELETE_TRIGGER_SQL,H as CREATE_BLOCKS_TYPE_INSERT_TRIGGER_SQL,U as CREATE_BLOCKS_TYPE_UPDATE_TRIGGER_SQL,O as CREATE_BLOCKS_UPDATE_ROW_EVENT_TRIGGER_SQL,M as CREATE_BLOCKS_UPLOAD_INSERT_TRIGGER_SQL,N as CREATE_BLOCKS_UPLOAD_UPDATE_TRIGGER_SQL,P as CREATE_BLOCKS_WORKSPACE_INVARIANT_INSERT_TRIGGER_SQL,F as CREATE_BLOCKS_WORKSPACE_INVARIANT_UPDATE_TRIGGER_SQL,f as CREATE_BLOCK_ALIASES_TABLE_SQL,V as CREATE_BLOCK_ALIASES_WORKSPACE_UNIQUE_TRIGGER_SQL,ee as CREATE_BLOCK_ALIASES_WS_ALIAS_INDEX_SQL,ne as CREATE_BLOCK_ALIASES_WS_ALIAS_LOWER_INDEX_SQL,m as CREATE_BLOCK_TYPES_TABLE_SQL,h as CREATE_BLOCK_TYPES_TYPE_WORKSPACE_INDEX_SQL,te as CREATE_CLIENT_SCHEMA_STATE_TABLE_SQL,u as CREATE_COMMAND_EVENTS_CREATED_INDEX_SQL,l as CREATE_COMMAND_EVENTS_TABLE_SQL,d as CREATE_COMMAND_EVENTS_WORKSPACE_INDEX_SQL,v as CREATE_PS_CRUD_REJECTED_REJECTED_AT_INDEX_SQL,_ as CREATE_PS_CRUD_REJECTED_TABLE_SQL,y as CREATE_PS_CRUD_REJECTED_TX_ID_INDEX_SQL,s as CREATE_ROW_EVENTS_BLOCK_INDEX_SQL,c as CREATE_ROW_EVENTS_CREATED_INDEX_SQL,a as CREATE_ROW_EVENTS_TABLE_SQL,o as CREATE_ROW_EVENTS_TX_INDEX_SQL,r as CREATE_TX_CONTEXT_TABLE_SQL,p as DROP_BLOCKS_WORKSPACE_TYPE_INDEX_SQL,Oe as RECONCILE_RESCAN_MARKER_PREFIX,pe as RECORD_BLOCKS_FTS_BACKFILL_DONE_SQL,se as RECORD_BLOCK_ALIASES_BACKFILL_DONE_SQL,ue as RECORD_BLOCK_TYPES_BACKFILL_DONE_SQL,Ae as RECORD_RECONCILE_RESCAN_MARKER_SQL,Ce as RECORD_REPROJECT_REF_MARKER_SQL,De as RECORD_WORKSPACE_BACKFILL_MARKER_SQL,xe as REPROJECT_REF_MARKER_PREFIX,i as SEED_TX_CONTEXT_ROW_SQL,be as SELECT_BLOCKS_COUNT_SQL,fe as SELECT_BLOCKS_FTS_BACKFILL_DONE_SQL,ve as SELECT_BLOCKS_STAT_ESTIMATE_SQL,X as SELECT_BLOCK_ALIASES_BACKFILL_DONE_SQL,le as SELECT_BLOCK_TYPES_BACKFILL_DONE_SQL,ke as SELECT_RECONCILE_RESCAN_MARKER_SQL,Se as SELECT_REPROJECT_REF_MARKERS_SQL,ye as SELECT_SQLITE_STAT1_EXISTS_SQL,Ee as SELECT_WORKSPACE_BACKFILL_MARKERS_SQL,Te as WORKSPACE_BACKFILL_MARKER_PREFIX,Ve as analyzeIsWarranted,Ie as backfillBlockAliasesIfEmpty,Le as backfillBlockTypesIfEmpty,Re as backfillBlocksFtsIfEmpty,ze as ensureBlockUserUpdatedAtColumn,$ as getBlocksCount,Be as getBlocksStatEstimate,He as runAnalyzeIfStale,Ue as runAnalyzeNow};
