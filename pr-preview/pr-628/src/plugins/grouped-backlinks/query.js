...
`,L=e({name:C,argsSchema:n({workspaceId:r(),id:r(),filter:i.optional(),groupingConfig:T}),resultSchema:w,resolve:async({workspaceId:e,id:t,filter:n,groupingConfig:r},i)=>{if(!e||!t)return{groups:[],total:0,unfilteredSourceIds:[],sourceParents:[]};let o=_(n),s=x(r??y);i.depend({kind:`plugin`,channel:u,key:m(e,t)}),i.depend({kind:`plugin`,channel:c,key:f(e,a.name)});let l=await k(i,e,t),d=g(o)?await k(i,e,t,o):l;if(d.length===0)return{groups:[],total:0,unfilteredSourceIds:l,sourceParents:[]};let p=await A(i,e,d),v=JSON.stringify(d),C=await i.db.getAll(M,[v]);i.hydrateBlocks(E(C),{declareRowDeps:!1});let w=await i.db.getAll(N,[v,e,t]),T=await i.db.getAll(P,[v,e,t]),D=new Map;for(let e of w){let t=D.get(e.id);t||(t=new Set,D.set(e.id,t)),t.add(e.source_id)}let j=Array.from(D.keys()),L=j.length===0?`[]`:JSON.stringify(j),R=j.length===0?[]:await i.db.getAll(F,[L,e,b,t]),z=j.length===0?[]:await i.db.getAll(I,[L,e,e,e]),B=i.repo.types,V=new Map;for(let e of B.values())V.set(e.id,e.label??e.id);let H=new Map;for(let e of w)H.set(e.id,e);for(let e of R)H.has(e.id)||H.set(e.id,e);let U=i.hydrateBlocks(E(Array.from(H.values())),{declareRowDeps:!1});for(let e of U)O(i,e.workspaceId,e.id);let W=new Map(U.map(e=>[e.id,h(e,e.id)])),G=w.map(e=>({sourceId:e.source_id,groupId:e.id,groupLabel:W.get(e.id)??(e.content.trim()||e.id),kind:e.context_kind===`root`?`root`:`ref`}));for(let e of T)G.push({sourceId:e.source_id,groupId:`field:${e.source_field}`,groupLabel:e.source_field,kind:`field`});for(let e of R){let t=D.get(e.context_id);if(!t)continue;let n=W.get(e.id)??(e.content.trim()||e.id);for(let r of t)G.push({sourceId:r,groupId:e.id,groupLabel:n,kind:`attribute`})}let K=new Set;for(let e of z){let t=D.get(e.context_id);if(!t)continue;let n=`type:${e.type_name}`,r=V.get(e.type_name)??e.type_name;for(let e of t){let t=`${e}\x00${n}`;K.has(t)||(K.add(t),G.push({sourceId:e,groupId:n,groupLabel:r,kind:`type`}))}}return{groups:S({targetId:t,sourceOrder:d,candidates:G,groupingConfig:s}),total:d.length,unfilteredSourceIds:l,sourceParents:p}}});export{C as GROUPED_BACKLINKS_FOR_BLOCK_QUERY,F as SELECT_GROUPED_BACKLINK_ATTRIBUTE_CANDIDATES_SQL,N as SELECT_GROUPED_BACKLINK_CANDIDATES_SQL,P as SELECT_GROUPED_BACKLINK_FIELD_CANDIDATES_SQL,M as SELECT_GROUPED_BACKLINK_MEMBER_ROWS_SQL,I as SELECT_GROUPED_BACKLINK_TYPE_CANDIDATES_SQL,L as groupedBacklinksForBlockQuery};
