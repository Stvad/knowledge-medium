//#region src/plugins/default-themes/themes.ts
var SUNSET_WARM_LIGHT = {
	id: "sunset-warm-light",
	label: "Sunset Warm Light",
	mode: "light",
	tokens: {
		background: "30 40% 99%",
		foreground: "20 30% 12%",
		card: "0 0% 100%",
		"card-foreground": "20 30% 12%",
		popover: "0 0% 100%",
		"popover-foreground": "20 30% 12%",
		primary: "14 90% 53%",
		"primary-foreground": "30 40% 99%",
		secondary: "20 60% 95%",
		"secondary-foreground": "14 70% 30%",
		muted: "20 25% 95%",
		"muted-foreground": "20 12% 42%",
		accent: "5 80% 92%",
		"accent-foreground": "5 70% 30%",
		destructive: "0 84% 55%",
		"destructive-foreground": "0 0% 98%",
		border: "20 25% 88%",
		input: "20 25% 86%",
		ring: "14 90% 53%",
		link: "14 90% 53%",
		wikilink: "0 75% 50%",
		code: "20 30% 95%",
		success: "142 55% 38%",
		radius: "0.65rem",
		"chart-1": "14 90% 53%",
		"chart-2": "5 80% 60%",
		"chart-3": "35 90% 55%",
		"chart-4": "280 60% 60%",
		"chart-5": "180 60% 45%"
	}
};
var SUNSET_WARM_DARK = {
	id: "sunset-warm-dark",
	label: "Sunset Warm Dark",
	mode: "dark",
	tokens: {
		background: "20 30% 7%",
		foreground: "30 20% 95%",
		card: "20 30% 9%",
		"card-foreground": "30 20% 95%",
		popover: "20 30% 9%",
		"popover-foreground": "30 20% 95%",
		primary: "14 90% 60%",
		"primary-foreground": "20 30% 7%",
		secondary: "20 30% 16%",
		"secondary-foreground": "20 60% 92%",
		muted: "20 20% 14%",
		"muted-foreground": "20 12% 65%",
		accent: "5 55% 25%",
		"accent-foreground": "5 80% 92%",
		destructive: "0 62% 45%",
		"destructive-foreground": "0 0% 98%",
		border: "20 25% 18%",
		input: "20 25% 18%",
		ring: "14 90% 60%",
		link: "14 90% 70%",
		wikilink: "0 85% 70%",
		code: "20 25% 18%",
		success: "142 50% 50%",
		radius: "0.65rem",
		"chart-1": "14 90% 60%",
		"chart-2": "5 80% 65%",
		"chart-3": "35 90% 60%",
		"chart-4": "280 60% 65%",
		"chart-5": "180 60% 55%"
	}
};
/** Cycle order. Sunset Warm comes first because it's the runtime
*  default (see the pre-paint script in index.html); the light/dark
*  variant of each family are siblings so a single toggle from any
*  theme hits its dark/light counterpart before moving to another
*  family. */
var defaultThemeContributions = [
	SUNSET_WARM_LIGHT,
	SUNSET_WARM_DARK,
	{
		id: "light",
		label: "Light",
		mode: "light",
		tokens: {
			background: "0 0% 100%",
			foreground: "0 0% 3.9%",
			card: "0 0% 100%",
			"card-foreground": "0 0% 3.9%",
			popover: "0 0% 100%",
			"popover-foreground": "0 0% 3.9%",
			primary: "0 0% 9%",
			"primary-foreground": "0 0% 98%",
			secondary: "0 0% 96.1%",
			"secondary-foreground": "0 0% 9%",
			muted: "0 0% 96.1%",
			"muted-foreground": "0 0% 45.1%",
			accent: "0 0% 96.1%",
			"accent-foreground": "0 0% 9%",
			destructive: "0 84.2% 60.2%",
			"destructive-foreground": "0 0% 98%",
			border: "0 0% 89.8%",
			input: "0 0% 89.8%",
			ring: "0 0% 3.9%",
			link: "217 91% 60%",
			wikilink: "271 81% 56%",
			code: "220 14% 96%",
			success: "142 71% 38%",
			radius: "0.5rem",
			"chart-1": "12 76% 61%",
			"chart-2": "173 58% 39%",
			"chart-3": "197 37% 24%",
			"chart-4": "43 74% 66%",
			"chart-5": "27 87% 67%"
		}
	},
	{
		id: "dark",
		label: "Dark",
		mode: "dark",
		tokens: {
			background: "0 0% 3.9%",
			foreground: "0 0% 98%",
			card: "0 0% 3.9%",
			"card-foreground": "0 0% 98%",
			popover: "0 0% 3.9%",
			"popover-foreground": "0 0% 98%",
			primary: "0 0% 98%",
			"primary-foreground": "0 0% 9%",
			secondary: "0 0% 14.9%",
			"secondary-foreground": "0 0% 98%",
			muted: "0 0% 14.9%",
			"muted-foreground": "0 0% 63.9%",
			accent: "0 0% 14.9%",
			"accent-foreground": "0 0% 98%",
			destructive: "0 62.8% 30.6%",
			"destructive-foreground": "0 0% 98%",
			border: "0 0% 14.9%",
			input: "0 0% 14.9%",
			ring: "0 0% 83.1%",
			link: "217 91% 60%",
			wikilink: "270 95% 75%",
			code: "216 31% 17%",
			success: "142 65% 50%",
			radius: "0.5rem",
			"chart-1": "220 70% 50%",
			"chart-2": "160 60% 45%",
			"chart-3": "30 80% 55%",
			"chart-4": "280 65% 60%",
			"chart-5": "340 75% 55%"
		}
	},
	{
		id: "indigo-light",
		label: "Indigo Light",
		mode: "light",
		tokens: {
			background: "240 25% 99%",
			foreground: "240 30% 12%",
			card: "0 0% 100%",
			"card-foreground": "240 30% 12%",
			popover: "0 0% 100%",
			"popover-foreground": "240 30% 12%",
			primary: "243 75% 59%",
			"primary-foreground": "240 25% 99%",
			secondary: "243 30% 95%",
			"secondary-foreground": "243 60% 25%",
			muted: "240 20% 96%",
			"muted-foreground": "240 12% 45%",
			accent: "270 80% 92%",
			"accent-foreground": "270 60% 25%",
			destructive: "0 84% 60%",
			"destructive-foreground": "0 0% 98%",
			border: "240 18% 90%",
			input: "240 18% 88%",
			ring: "243 75% 59%",
			link: "243 75% 59%",
			wikilink: "280 75% 55%",
			code: "240 25% 95%",
			success: "142 65% 42%",
			radius: "0.6rem",
			"chart-1": "243 75% 59%",
			"chart-2": "270 70% 60%",
			"chart-3": "199 80% 55%",
			"chart-4": "330 70% 65%",
			"chart-5": "35 90% 60%"
		}
	},
	{
		id: "indigo-dark",
		label: "Indigo Dark",
		mode: "dark",
		tokens: {
			background: "240 25% 7%",
			foreground: "240 15% 95%",
			card: "240 25% 9%",
			"card-foreground": "240 15% 95%",
			popover: "240 25% 9%",
			"popover-foreground": "240 15% 95%",
			primary: "243 75% 65%",
			"primary-foreground": "240 25% 7%",
			secondary: "243 25% 16%",
			"secondary-foreground": "243 30% 90%",
			muted: "240 18% 14%",
			"muted-foreground": "240 12% 65%",
			accent: "270 50% 25%",
			"accent-foreground": "270 80% 92%",
			destructive: "0 62% 45%",
			"destructive-foreground": "0 0% 98%",
			border: "240 20% 18%",
			input: "240 20% 18%",
			ring: "243 75% 65%",
			link: "243 85% 75%",
			wikilink: "280 85% 75%",
			code: "240 25% 18%",
			success: "142 60% 55%",
			radius: "0.6rem",
			"chart-1": "243 75% 65%",
			"chart-2": "270 70% 70%",
			"chart-3": "199 80% 60%",
			"chart-4": "330 70% 70%",
			"chart-5": "35 90% 65%"
		}
	},
	{
		id: "solarized-light",
		label: "Solarized Light",
		mode: "light",
		tokens: {
			background: "44 87% 94%",
			foreground: "196 13% 45%",
			card: "44 87% 94%",
			"card-foreground": "196 13% 45%",
			popover: "44 87% 94%",
			"popover-foreground": "196 13% 45%",
			primary: "205 69% 49%",
			"primary-foreground": "44 87% 94%",
			secondary: "46 42% 88%",
			"secondary-foreground": "194 14% 40%",
			muted: "46 42% 88%",
			"muted-foreground": "180 9% 60%",
			accent: "205 50% 90%",
			"accent-foreground": "194 14% 40%",
			destructive: "1 71% 52%",
			"destructive-foreground": "44 87% 94%",
			border: "46 30% 80%",
			input: "46 30% 80%",
			ring: "205 69% 49%",
			link: "205 69% 49%",
			wikilink: "237 43% 60%",
			code: "46 42% 88%",
			success: "68 100% 30%",
			radius: "0.5rem",
			"chart-1": "18 80% 44%",
			"chart-2": "68 100% 30%",
			"chart-3": "175 59% 40%",
			"chart-4": "331 64% 52%",
			"chart-5": "237 43% 60%"
		}
	},
	{
		id: "solarized-dark",
		label: "Solarized Dark",
		mode: "dark",
		tokens: {
			background: "192 100% 11%",
			foreground: "186 8% 55%",
			card: "192 100% 11%",
			"card-foreground": "186 8% 55%",
			popover: "192 100% 11%",
			"popover-foreground": "186 8% 55%",
			primary: "205 69% 49%",
			"primary-foreground": "192 100% 11%",
			secondary: "192 81% 14%",
			"secondary-foreground": "180 9% 60%",
			muted: "192 81% 14%",
			"muted-foreground": "194 14% 40%",
			accent: "200 60% 22%",
			"accent-foreground": "180 9% 60%",
			destructive: "1 71% 52%",
			"destructive-foreground": "44 87% 94%",
			border: "192 60% 18%",
			input: "192 60% 18%",
			ring: "205 75% 55%",
			link: "205 75% 60%",
			wikilink: "237 55% 70%",
			code: "192 81% 14%",
			success: "68 80% 50%",
			radius: "0.5rem",
			"chart-1": "18 80% 50%",
			"chart-2": "68 80% 45%",
			"chart-3": "175 59% 50%",
			"chart-4": "331 64% 60%",
			"chart-5": "237 55% 65%"
		}
	}
];
/** The id pre-paint falls back to when nothing is persisted and the
*  system preference is "light". Kept in sync with index.html. */
var DEFAULT_THEME_ID_LIGHT = SUNSET_WARM_LIGHT.id;
/** The id pre-paint falls back to when nothing is persisted and the
*  system preference is "dark". */
var DEFAULT_THEME_ID_DARK = SUNSET_WARM_DARK.id;
//#endregion
export { DEFAULT_THEME_ID_DARK, DEFAULT_THEME_ID_LIGHT, defaultThemeContributions };

//# sourceMappingURL=themes.js.map