//#region src/data/internals/treeQueries.ts
/**
...
var manyAncestorsSql = (idCount) => {
    ...
    WITH RECURSIVE chain AS (
      SELECT blocks.*,
             ...
             0 AS depth
        FROM blocks
       WHERE blocks.id IN (${Array(idCount).fill("?").join(", ")}) AND blocks.deleted = 0
      UNION ALL
      SELECT parent.*,
...
var CHILDREN_IDS_SQL = `
  SELECT id FROM blocks
   WHERE parent_id = ? AND deleted = 0
   ORDER BY order_key, id
`;
//#endregion
export { ANCESTORS_SQL, CHILDREN_IDS_SQL, CHILDREN_SQL, IS_DESCENDANT_OF_SQL, SUBTREE_SQL, cycleScanSql, manyAncestorsSql };
