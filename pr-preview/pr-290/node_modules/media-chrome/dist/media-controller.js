import { AttributeToStateChangeEventMap, MediaStateReceiverAttributes, MediaUIAttributes, MediaUIEvents, MediaUIProps } from "./constants.js";
import { delay, stringifyAudioTrackList, stringifyRenditionList } from "./utils/utils.js";
...
var MediaController = class extends MediaContainer {
	constructor() {
		...
		__privateAdd(this, _mediaStateEventHandler, (event) => {
			var _a;
			(_a = __privateGet(this, _mediaStore)) == null || _a.dispatch(event);
		});
		__privateAdd(this, _subtitlesState, void 0);
		__privateAdd(this, _keyUpHandler, (e) => {
			const { key, shiftKey } = e;
			if (!(shiftKey && (key === "/" || key === "?") || ButtonPressedKeys.includes(key))) {
	...
	set mediaStore(value) {
		...
		if (__privateGet(this, _mediaStore)) {
			(_a = __privateGet(this, _mediaStoreUnsubscribe)) == null || _a.call(this);
	...
	set fullscreenElement(element) {
		var _a;
		if (this.hasAttribute(Attributes.FULLSCREEN_ELEMENT)) this.removeAttribute(Attributes.FULLSCREEN_ELEMENT);
		__privateSet(this, _fullscreenElement, element);
		(_a = __privateGet(this, _mediaStore)) == null || _a.dispatch({
	...
	attributeChangedCallback(attrName, oldValue, newValue) {
		...
		if (attrName === Attributes.NO_HOTKEYS) {
			if (newValue !== oldValue && newValue === "") {
				if (this.hasAttribute(Attributes.HOTKEYS)) console.warn("Media Chrome: Both `hotkeys` and `nohotkeys` have been set. All hotkeys will be disabled.");
				this.disableHotkeys();
			} else if (newValue !== oldValue && newValue === null) this.enableHotkeys();
		} else if (attrName === Attributes.HOTKEYS) __privateGet(this, _hotKeys).value = newValue;
		else if (attrName === Attributes.DEFAULT_SUBTITLES && newValue !== oldValue) (_a = __privateGet(this, _mediaStore)) == null || _a.dispatch({
			type: "optionschangerequest",
			detail: { defaultSubtitles: this.hasAttribute(Attributes.DEFAULT_SUBTITLES) }
		});
		else if (attrName === Attributes.DEFAULT_STREAM_TYPE) (_c = __privateGet(this, _mediaStore)) == null || _c.dispatch({
			type: "optionschangerequest",
			detail: { defaultStreamType: (_b = this.getAttribute(Attributes.DEFAULT_STREAM_TYPE)) != null ? _b : void 0 }
		});
		else if (attrName === Attributes.LIVE_EDGE_OFFSET && newValue !== oldValue) (_d = __privateGet(this, _mediaStore)) == null || _d.dispatch({
			type: "optionschangerequest",
			detail: {
		...
		else if (attrName === Attributes.SEEK_TO_LIVE_OFFSET && newValue !== oldValue) (_e = __privateGet(this, _mediaStore)) == null || _e.dispatch({
			type: "optionschangerequest",
			detail: { seekToLiveOffset: this.hasAttribute(Attributes.SEEK_TO_LIVE_OFFSET) ? +this.getAttribute(Attributes.SEEK_TO_LIVE_OFFSET) : this.hasAttribute(Attributes.LIVE_EDGE_OFFSET) ? +this.getAttribute(Attributes.LIVE_EDGE_OFFSET) : void 0 }
		});
		else if (attrName === Attributes.NO_AUTO_SEEK_TO_LIVE) (_f = __privateGet(this, _mediaStore)) == null || _f.dispatch({
			type: "optionschangerequest",
			detail: { noAutoSeekToLive: this.hasAttribute(Attributes.NO_AUTO_SEEK_TO_LIVE) }
		});
			...
			(_h = __privateGet(this, _mediaStore)) == null || _h.dispatch({
				type: "fullscreenelementchangerequest",
				detail: this.fullscreenElement
			});
		} else if (attrName === Attributes.LANG && newValue !== oldValue) {
			setLanguage(newValue);
			(_i = __privateGet(this, _mediaStore)) == null || _i.dispatch({
				type: "optionschangerequest",
				detail: { mediaLang: newValue }
			});
		} else if (attrName === Attributes.LOOP && newValue !== oldValue) (_j = __privateGet(this, _mediaStore)) == null || _j.dispatch({
			type: MediaUIEvents.MEDIA_LOOP_REQUEST,
			detail: newValue != null
		});
		else if (attrName === Attributes.NO_VOLUME_PREF && newValue !== oldValue) (_k = __privateGet(this, _mediaStore)) == null || _k.dispatch({
			type: "optionschangerequest",
			detail: { noVolumePref: this.hasAttribute(Attributes.NO_VOLUME_PREF) }
		});
		else if (attrName === Attributes.NO_MUTED_PREF && newValue !== oldValue) (_l = __privateGet(this, _mediaStore)) == null || _l.dispatch({
			type: "optionschangerequest",
			detail: { noMutedPref: this.hasAttribute(Attributes.NO_MUTED_PREF) }
		});
	}
	connectedCallback() {
		var _a, _b, _c;
		this.associateElement(this);
		if (!__privateGet(this, _mediaStore) && !this.hasAttribute(Attributes.NO_DEFAULT_STORE)) __privateMethod(this, _setupDefaultStore, setupDefaultStore_fn).call(this);
		(_a = __privateGet(this, _mediaStore)) == null || _a.dispatch({
			type: "documentelementchangerequest",
			detail: Document
		});
		(_b = __privateGet(this, _mediaStore)) == null || _b.dispatch({
			type: "fullscreenelementchangerequest",
			detail: this.fullscreenElement
		});
		super.connectedCallback();
		if (__privateGet(this, _mediaStore) && !__privateGet(this, _mediaStoreUnsubscribe)) __privateSet(this, _mediaStoreUnsubscribe, (_c = __privateGet(this, _mediaStore)) == null ? void 0 : _c.subscribe(__privateGet(this, _mediaStateCallback)));
		if (__privateGet(this, _subtitlesState) !== void 0 && __privateGet(this, _mediaStore) && this.media) setTimeout(() => {
			var _a2, _b2, _c2;
			if ((_b2 = (_a2 = this.media) == null ? void 0 : _a2.textTracks) == null ? void 0 : _b2.length) (_c2 = __privateGet(this, _mediaStore)) == null || _c2.dispatch({
	...
	disconnectedCallback() {
		...
		if (__privateGet(this, _mediaStore)) {
			...
			(_c = __privateGet(this, _mediaStore)) == null || _c.dispatch({
				type: "fullscreenelementchangerequest",
				detail: void 0
			});
			(_d = __privateGet(this, _mediaStore)) == null || _d.dispatch({
				type: "documentelementchangerequest",
				detail: void 0
			});
			(_e = __privateGet(this, _mediaStore)) == null || _e.dispatch({
				type: MediaUIEvents.MEDIA_TOGGLE_SUBTITLES_REQUEST,
				detail: false
			});
		}
		if (__privateGet(this, _mediaStoreUnsubscribe)) {
			(_f = __privateGet(this, _mediaStoreUnsubscribe)) == null || _f.call(this);
			__privateSet(this, _mediaStoreUnsubscribe, void 0);
		}
	...
	*/
	...
	registerMediaStateReceiver(el) {
		...
		els.push(el);
		if (__privateGet(this, _mediaStore)) Object.entries(__privateGet(this, _mediaStore).getState()).forEach(([stateName, stateValue]) => {
	...
	keyboardShortcutHandler(e) {
		...
		if (((_c = (_b = (_a = target.getAttribute(Attributes.KEYS_USED)) == null ? void 0 : _a.split(" ")) != null ? _b : target == null ? void 0 : target.keysUsed) != null ? _c : []).map((key) => key === "Space" ? " " : key).filter(Boolean).includes(e.key)) return;
		...
		if (e.shiftKey && (e.key === "/" || e.key === "?") && __privateGet(this, _hotKeys).contains("noshift+/")) return;
		switch (e.key) {
			case " ":
			case "k":
			...
			case "/":
			case "?":
				if (e.shiftKey) __privateMethod(this, _showKeyboardShortcutsDialog, showKeyboardShortcutsDialog_fn).call(this);
				break;
			case "p":
...
setupDefaultStore_fn = function() {
	...
	this.mediaStore = createMediaStore({
		...
		options: {
			...
			defaultStreamType: (_a = this.getAttribute(Attributes.DEFAULT_STREAM_TYPE)) != null ? _a : void 0,
			liveEdgeOffset: this.hasAttribute(Attributes.LIVE_EDGE_OFFSET) ? +this.getAttribute(Attributes.LIVE_EDGE_OFFSET) : void 0,
			seekToLiveOffset: this.hasAttribute(Attributes.SEEK_TO_LIVE_OFFSET) ? +this.getAttribute(Attributes.SEEK_TO_LIVE_OFFSET) : this.hasAttribute(Attributes.LIVE_EDGE_OFFSET) ? +this.getAttribute(Attributes.LIVE_EDGE_OFFSET) : void 0,
...
keyDownHandler_fn = function(e) {
	...
	if ([
		...
		"ArrowUp",
		"ArrowDown"
	].includes(key) && !(__privateGet(this, _hotKeys).contains(`no${key.toLowerCase()}`) || key === " " && __privateGet(this, _hotKeys).contains("nospace")) && !isRangeInput) e.preventDefault();
...
//#endregion
export { media_controller_default };
