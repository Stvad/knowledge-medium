...
async function asyncModuleFactory(encryptionKey) {
	if (encryptionKey) {
		const { default: factory } = await import("./mc-wa-sqlite-async-DG3fW6Mq.js");
		return factory();
	} else {
		const { default: factory } = await import("./wa-sqlite-async-DlqBdP8I.js");
		return factory();
	}
}
async function syncModuleFactory(encryptionKey) {
	if (encryptionKey) {
		const { default: factory } = await import("./mc-wa-sqlite-DBPRv5CV.js");
		return factory();
	} else {
		const { default: factory } = await import("./wa-sqlite-DSyJSLGk.js");
		return factory();
	}
}
/**
...
const DEFAULT_MODULE_FACTORIES = {
	[WASQLiteVFS.IDBBatchAtomicVFS]: async (options) => {
		...
		const { IDBBatchAtomicVFS } = await import("./IDBBatchAtomicVFS-C9oKoTwM.js");
		return {
			module,
	...
	[WASQLiteVFS.AccessHandlePoolVFS]: async (options) => {
		...
		const { AccessHandlePoolVFS } = await import("./AccessHandlePoolVFS-CLPMQw2R.js");
		return {
			module,
	...
	[WASQLiteVFS.OPFSCoopSyncVFS]: async (options) => {
		const module = await syncModuleFactory(options.encryptionKey);
		const { OPFSCoopSyncVFS } = await import("./OPFSCoopSyncVFS-DQoltAZj.js");
		return {
			module,
	...
	[WASQLiteVFS.OPFSWriteAheadVFS]: async (options) => {
		const module = await syncModuleFactory(options.encryptionKey);
		const { OPFSWriteAheadVFS } = await import("./OPFSWriteAheadVFS-CZibJo88.js");
		return {
			module,
...
//# sourceMappingURL=WASQLiteDB.worker-BEqhjpXD.js.map