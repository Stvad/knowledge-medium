//#region src/data/syncedTableWriteGuard.ts
/**
...
var unquote = (ident) => ident.replace(/^["'`[]/, "").replace(/["'`\]]$/, "");
/**
...
var guardSyncedTableWrites = (execute) => (sql, ...rest) => {
	const target = writeTargetTable(sql);
	if (target !== null && SYNCED_TABLES.has(target)) return Promise.reject(/* @__PURE__ */ new Error(`[syncedTableWriteGuard] refusing a raw write to synced table "${target}". Backfill / local-schema writes to blocks/workspaces/workspace_members leave tx_context.source = NULL, so the upload trigger never fires and the write is local-only — it will silently never sync (see the daily-note:date backfill, gone in 8c50f167). Route synced-table backfills through repo.tx — e.g. workspaceBackfillsFacet — which uploads. Offending SQL: ${sql.trim().slice(0, 120)}`));
	return execute(sql, ...rest);
};
//#endregion
export { guardSyncedTableWrites, writeTargetTable };
