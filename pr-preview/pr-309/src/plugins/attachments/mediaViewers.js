...
var DownloadIconButton = (t0) => {
	const $ = c(8);
	const { resolveBytes, filename, label, className } = t0;
	const { status, download } = useMediaDownload(resolveBytes, filename);
	const t1 = status === "resolving";
	const t2 = status === "error" ? `${label} — download failed, click to retry` : `Download ${label}`;
	...
	let t5;
	if ($[2] !== download || $[3] !== t1 || $[4] !== t2 || $[5] !== t3 || $[6] !== t4) {
...
//#endregion
export { FILE_VIEWER_FALLBACK, audioMediaViewer, formatByteSize, imageMediaViewer, pickMediaViewer };
