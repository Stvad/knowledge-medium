...
import { media_gesture_receiver_default } from "./media-gesture-receiver.js";
...
function setBreakpoints(container, width) {
	...
	const ranges = createBreakpointMap((_a = container.getAttribute(Attributes.BREAKPOINTS)) != null ? _a : defaultBreakpoints);
...
var MediaContainer = class extends GlobalThis.HTMLElement {
	constructor() {
		...
		__privateAdd(this, _handleSlotChange, () => {
			if (!__privateGet(this, _chainedSlot).assignedElements({ flatten: true }).length) {
				if (__privateGet(this, _currentMedia)) this.mediaUnsetCallback(__privateGet(this, _currentMedia));
				return;
			}
...
handlePointerMove_fn = function(event) {
	if (event.pointerType !== "mouse") {
		if (event.timeStamp - __privateGet(this, _pointerDownTimeStamp) < 250) return;
	}
	...
	if ([this, this.media].includes(event.target) || autohideOverControls) __privateMethod(this, _scheduleInactive, scheduleInactive_fn).call(this);
};
...
handlePointerUp_fn = function(event) {
	if (event.pointerType === "touch") {
		...
		if ([this, this.media].includes(event.target) && controlsVisible) __privateMethod(this, _setInactive, setInactive_fn).call(this);
		else __privateMethod(this, _scheduleInactive, scheduleInactive_fn).call(this);
	} else if (event.composedPath().some((el) => ["media-play-button", "media-fullscreen-button"].includes(el == null ? void 0 : el.localName))) __privateMethod(this, _scheduleInactive, scheduleInactive_fn).call(this);
};
...
//#endregion
export { Attributes, MediaContainer };
