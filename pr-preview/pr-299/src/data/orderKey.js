import { generateJitteredKeyBetween, generateNJitteredKeysBetween } from "../../node_modules/fractional-indexing-jittered/lib/index.js";
//#region src/data/orderKey.ts
/**
...
*/
var keyBetween = (lower, upper) => generateJitteredKeyBetween(lower, upper);
/** N new keys strictly between `lower` and `upper`. Returned in
*  ascending order; safe to insert as a contiguous run of siblings. */
var keysBetween = (lower, upper, n) => generateNJitteredKeysBetween(lower, upper, n);
/** First key — for an empty sibling list, or to put a new row at the
*  very start. */
var keyAtStart = (firstExisting = null) => keyBetween(null, firstExisting);
/** Last key — for an empty sibling list, or to put a new row at the
*  very end. */
var keyAtEnd = (lastExisting = null) => keyBetween(lastExisting, null);
//#endregion
export { keyAtEnd, keyAtStart, keyBetween, keysBetween };
