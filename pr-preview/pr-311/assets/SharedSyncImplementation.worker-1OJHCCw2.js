...
var TransactionImplementation = class TransactionImplementation extends DBGetUtilsDefaultMixin(BaseTransaction) {
...
var SyncProgress = class {
	...
	*/
	untilPriority(priority) {
		let total = 0;
		let downloaded = 0;
		for (const progress of Object.values(this.internal)) if (progress.priority <= priority) {
			downloaded += progress.since_last;
			total += progress.target_count - progress.at_last;
		}
		let progress = total == 0 ? 0 : downloaded / total;
		return {
...
* @public
*/
var SyncStatus = class SyncStatus {
	options;
	constructor(options) {
	...
	*/
	get downloadProgress() {
		...
		if (internalProgress == null) return null;
	...
	*/
	statusForPriority(priority) {
		for (const known of this.priorityStatusEntries) if (known.priority >= priority) return known;
		return {
			priority,
...
var SyncStreamStatusView = class {
	status;
	core;
	subscription;
	constructor(status, core) {
		...
		this.subscription = {
			...
			expiresAt: core.expires_at != null ? /* @__PURE__ */ new Date(core.expires_at * 1e3) : null,
			hasSynced: core.last_synced_at != null,
			lastSyncedAt: core.last_synced_at != null ? /* @__PURE__ */ new Date(core.last_synced_at * 1e3) : null
		};
	}
	get progress() {
		if (this.status.dataFlowStatus.downloadProgress == null) return null;
		const { total, downloaded } = this.core.progress;
		return {
			totalOperations: total,
			downloadedOperations: downloaded,
			downloadedFraction: total == 0 ? 0 : downloaded / total
		};
	}
	get priority() {
...
var BaseObserver = class {
	...
	*/
	...
	iterateListeners(cb) {
		for (const listener of this.listeners) cb(listener);
	}
	async iterateAsyncListeners(cb) {
		for (let i of Array.from(this.listeners.values())) await cb(i);
	}
};
/**
...
function injectable(source) {
	...
	const fetchFromSource = () => {
		const resolveWaiter = (propagate) => {
			...
			if (active) propagate(active);
			else pendingSourceEvent = propagate;
		};
	...
	return {
		next: () => {
			return new Promise((resolve, reject) => {
				if (sourceIsDone) return resolve(doneResult);
				...
				if (pendingInjectedEvents.length) return resolve(valueResult(pendingInjectedEvents.shift()));
				waiter = {
					resolve,
					reject
				};
				if (!sourceFetchInFlight) fetchFromSource();
			});
		},
		inject: (event) => {
			const pending = consumeWaiter();
			if (pending != null) pending.resolve(valueResult(event));
			else pendingInjectedEvents.push(event);
		}
	};
}
/**
...
function extractJsonLines(source, decoder) {
	...
	return { next: async () => {
		while (true) {
			if (isFinalEvent) return doneResult;
			{
...
function extractBsonObjects(source) {
	...
	let isDone = false;
	const lengthBuffer = /* @__PURE__ */ new DataView(/* @__PURE__ */ new ArrayBuffer(4));
	let objectBody = null;
	let remainingLength = 4;
	return { async next() {
		while (true) {
			if (completedObjects.length) return valueResult(completedObjects.shift());
			if (isDone) return doneResult;
			const upstreamEvent = await source.next();
			if (upstreamEvent.done) {
				isDone = true;
				if (objectBody || remainingLength != 4) throw new Error("illegal end of stream in BSON object");
				return doneResult;
			}
			...
			for (let i = 0; i < chunk.length;) {
				...
				if (objectBody) {
					...
					const bytesToRead = Math.min(availableInData, remainingLength);
					for (let j = 0; j < bytesToRead; j++) lengthBuffer.setUint8(4 - remainingLength + j, chunk[i + j]);
					i += bytesToRead;
					remainingLength -= bytesToRead;
					if (remainingLength == 0) {
						const length = lengthBuffer.getInt32(0, true);
						remainingLength = length - 4;
						if (remainingLength < 1) throw new Error(`invalid length for bson: ${length}`);
...
var EventQueue = class EventQueue {
	...
	*/
	...
	notifyInner(dispatch) {
		...
		if (existing) dispatchAndNotifyListeners(existing);
		else this.outstandingEvents.push(dispatchAndNotifyListeners);
	}
	notify(value) {
	...
	waitForEvent(signal) {
		return new Promise((resolve, reject) => {
			if (this.waitingConsumer != null) throw new Error("Illegal call to waitForEvent, already has a waiter.");
			const complete = () => {
	...
	*/
	static queueBasedAsyncIterable(run, abort) {
		return { [symbolAsyncIterator]: () => {
			...
			if (abort) if (abort.aborted) controller.abort();
			else abort.addEventListener("abort", dispose);
...
* @internal
*/
var ConnectionManager = class extends BaseObserver {
	options;
	/**
	...
	*/
	locallyActiveSubscriptions = /* @__PURE__ */ new Map();
	constructor(options) {
		super();
		this.options = options;
	...
	async connect(connector, options) {
		...
		if (!hadPendingOptions || this.syncStreamImplementation) await this.disconnectInternal();
		const checkConnection = async () => {
			if (this.pendingConnectionOptions) {
				this.connectingPromise = this.connectInternal().catch(() => {}).finally(checkConnection);
	...
	stream(adapter, name, parameters) {
		...
		return {
			...
			subscribe: async (options) => {
				await adapter.rustSubscriptionsCommand({ subscribe: {
					stream: {
						name,
						params: parameters
					},
				...
				if (!this.syncStreamImplementation) await adapter.resolveOfflineSyncStatus();
			...
			unsubscribeAll: async () => {
				await adapter.rustSubscriptionsCommand({ unsubscribe: {
					name,
					params: parameters
				} });
				this.subscriptionsMayHaveChanged();
...
var ActiveSubscription = class {
	...
	decrementRefCount() {
		this.refcount--;
		if (this.refcount == 0) this.clearSubscription();
	}
};
var SyncStreamSubscriptionHandle = class {
	subscription;
	active = true;
...
const _finalizer = "FinalizationRegistry" in globalThis ? new FinalizationRegistry((sub) => {
...
* @internal
*/
var AbortOperation = class AbortOperation extends Error {
	reason;
	constructor(reason) {
		super(reason);
		this.reason = reason;
		Object.setPrototypeOf(this, AbortOperation.prototype);
		if (Error.captureStackTrace) Error.captureStackTrace(this, AbortOperation);
	}
};
var buffer = {};
var base64Js = {};
	...
	connect(multiplexerDemultiplexerFactory) {
		return new Promise((resolve, reject) => {
			...
			const errorListener = (event) => {
				...
				else if (ev.message != null) reject(/* @__PURE__ */ new Error(`Failed to create websocket connection: ${ev.message}`));
				else reject(/* @__PURE__ */ new Error(`Failed to create websocket connection to ${this.url}`));
			};
			/**
			...
			*/
			const closeListener = () => {
				removeListeners();
				reject(/* @__PURE__ */ new Error("WebSocket connection closed while opening"));
			};
			removeListeners = () => {
...
const KEEP_ALIVE_MS = 2e4;
const SOCKET_TIMEOUT_MS = 3e4;
const KEEP_ALIVE_LIFETIME_MS = 9e4;
/**
* @internal
*/
...
var AbstractRemote = class {
	...
	*/
	get fetch() {
		...
		return fetchImplementation instanceof FetchImplementationProvider ? fetchImplementation.getFetch() : fetchImplementation;
	}
	/**
	...
	*/
	async fetchCredentials() {
		...
		if (credentials?.endpoint.match(POWERSYNC_TRAILING_SLASH_MATCH)) throw new Error(`A trailing forward slash "/" was found in the fetchCredentials endpoint: "${credentials.endpoint}". Remove the trailing forward slash "/" to fix this error.`);
		return credentials;
	}
	/***
	...
	*/
	...
	async buildRequest(path) {
		...
		if (credentials != null && (credentials.endpoint == null || credentials.endpoint == "")) throw new Error("PowerSync endpoint not configured");
		else if (credentials?.token == null || credentials?.token == "") {
			const error = /* @__PURE__ */ new Error(`Not signed in`);
			error.status = 401;
			throw error;
		}
	...
	async post(path, data, headers = {}) {
		...
		if (res.status === 401) this.invalidateCredentials();
		if (!res.ok) throw new Error(`Received ${res.status} - ${res.statusText} when posting to ${path}: ${await res.text()}}`);
		return res.json();
	}
	async get(path, headers) {
		...
		if (res.status === 401) this.invalidateCredentials();
		if (!res.ok) throw new Error(`Received ${res.status} - ${res.statusText} when getting from ${path}: ${await res.text()}}`);
		return res.json();
	}
	/**
	...
	*/
	async socketStreamRaw(options) {
		...
		let rsocket = null;
		let paused = false;
		const queue = new EventQueue({ eventDelivered: () => {
		...
		const abortRequest = () => {
			...
			if (rsocket) rsocket.close();
			queue.notify(null);
		};
		function push(event) {
			queue.notify(event);
			if (queue.countOutstandingEvents >= SYNC_QUEUE_REQUEST_HIGH_WATER) paused = true;
		}
		...
		if (options.abortSignal.aborted) throw new AbortOperation("Connection request aborted");
		else options.abortSignal.addEventListener("abort", abortRequest);
		const resetTimeout = () => {
			clearTimeout(keepAliveTimeout);
		...
		return await new Promise((resolve, reject) => {
			const queueAsIterator = { next: async () => {
				if (didClose) return doneResult;
				const notification = await queue.waitForEvent(options.abortSignal);
				if (didClose) return doneResult;
				else return valueResult(notification);
			} };
			res = rsocket.requestStream({
				data: toBuffer(options.data),
				metadata: toBuffer({ path })
			}, syncQueueRequestSize, {
				onError: (e) => {
					if (e.message.includes("PSYNC_")) {
						if (e.message.includes("PSYNC_S21")) this.invalidateCredentials();
					} else if (e.message !== "Closed. ") this.invalidateCredentials();
					if (e.message !== "Closed. ") this.logger.error(e);
					abortRequest();
					if (!connectionEstablished) reject(e);
				},
				onNext: (payload) => {
	...
	*/
	async fetchStreamRaw(options) {
		...
		*/
		if (abortSignal.aborted) throw new AbortOperation("Abort request received before making fetchStreamRaw request");
		const controller = new AbortController();
		let reader = null;
		abortSignal.addEventListener("abort", () => {
			const reason = abortSignal.reason ?? new AbortOperation("Cancelling network request before it resolves. Abort signal has been received.");
			if (reader == null) controller.abort(reason);
			else reader.cancel(reason).catch(() => {});
		});
		let res;
		...
		try {
			...
			if (!res.ok || !res.body) {
				...
				const error = /* @__PURE__ */ new Error(`HTTP ${res.statusText}: ${text}`);
				error.status = res.status;
				throw error;
			}
			responseIsBson = res.headers.get("content-type") == bson;
		} catch (ex) {
			if (ex.name == "AbortError") throw new AbortOperation(`Pending fetch request to ${request.url} has been aborted.`);
			throw ex;
		}
		reader = res.body.getReader();
		return {
			isBson: responseIsBson,
			stream: { next: async () => {
				if (controller.signal.aborted) return doneResult;
				try {
	...
	*/
	async fetchStream(options) {
		...
		if (isBson) return extractBsonObjects(stream);
		else return extractJsonLines(stream, this.createTextDecoder());
	}
};
function priorityToJs(status) {
	return {
		priority: status.priority,
		hasSynced: status.has_synced ?? void 0,
		lastSyncedAt: status.last_synced_at != null ? /* @__PURE__ */ new Date(status.last_synced_at * 1e3) : void 0
	};
}
...
const DEFAULT_STREAM_CONNECTION_OPTIONS = {
	...
	params: {},
	serializedSchema: void 0,
	includeDefaultStreams: true
};
/**
* @internal
*/
var AbstractStreamingSyncImplementation = class extends BaseObserver {
	options;
	abortController;
	crudUpdateListener;
			...
			*/
			const matchPartialObject = (compA, compB) => {
				return Object.entries(compA).every(([key, value]) => {
					...
					if (typeof value == "object" && typeof comparisonBValue == "object") return matchPartialObject(value, comparisonBValue);
	...
	waitUntilStatusMatches(predicate) {
		return new Promise((resolve) => {
			...
			const l = this.registerListener({ statusChanged: (updatedStatus) => {
	...
	async getWriteCheckpoint() {
		let path = `/write-checkpoint2.json?client_id=${await this.options.adapter.getClientId()}`;
		const checkpoint = (await this.options.remote.get(path))["data"]["write_checkpoint"];
		this.logger.debug(`Created write checkpoint: ${checkpoint}`);
		return checkpoint;
	}
	async crudUploadLoop(signal) {
		while (!signal.aborted) {
			await Promise.all([this._uploadAllCrud(signal), this.delayRetry(signal, this.options.crudUploadThrottleMs)]);
				...
				*/
				...
				while (!signal.aborted) try {
					...
					*/
					...
					if (nextCrudItem) {
						this.updateSyncStatus({ dataFlow: { uploading: true } });
...
The next upload iteration will be delayed.`);
						...
						this.updateSyncStatus({ dataFlow: { uploadError: void 0 } });
					} else {
						if (await this.options.adapter.updateLocalTarget(() => this.getWriteCheckpoint())) this.notifyCompletedUploads?.();
						else if (checkedCrudItem != null) this.logger.debug("Upload complete, no write checkpoint needed.");
						break;
					}
				} catch (ex) {
					checkedCrudItem = void 0;
					this.updateSyncStatus({ dataFlow: {
						uploading: false,
						uploadError: ex
					} });
					...
					this.updateSyncStatus({ dataFlow: { uploading: false } });
				}
			}
		});
	}
	async connect(options) {
		if (this.abortController) await this.disconnect();
		const controller = new AbortController();
		this.abortController = controller;
		this.streamingSyncPromise = Promise.all([this.crudUploadLoop(controller.signal).catch((ex) => this.logger.error("Error in crud upload loop", ex)), this.streamingSync(controller.signal, options)]);
		return new Promise((resolve) => {
			const disposer = this.registerListener({ statusChanged: (status) => {
				if (status.dataFlowStatus.downloadError != null) this.logger.warn("Initial connect attempt did not successfully connect to server");
				else if (status.connecting) return;
				disposer();
				resolve();
			} });
		});
	...
	async disconnect() {
		...
		if (!this.abortController.signal.aborted) this.abortController.abort(new AbortOperation("Disconnect has been requested"));
		try {
	...
	async streamingSync(signal, options) {
		...
		*/
		this.crudUpdateListener = this.options.adapter.registerListener({ crudUpdate: () => this.triggerCrudUpload() });
		/**
			...
			try {
				...
				*/
				if (ex instanceof AbortOperation) {
					...
					shouldDelayRetry = false;
				} else this.logger.error(ex);
				this.updateSyncStatus({ dataFlow: { downloadError: ex } });
			} finally {
				...
				if (result?.immediateRestart != true) {
					...
					if (shouldDelayRetry) await this.delayRetry(nestedAbortController.signal);
				}
			}
		}
		this.updateSyncStatus({
	...
	*/
	...
	streamingSyncIteration(signal, options) {
		return this.obtainLock({
			...
			callback: async () => {
				...
				if (invalidMetadata.length > 0) throw new Error(`Invalid appMetadata provided. Only string values are allowed. Invalid values: ${invalidMetadata.map(([key, value]) => `${key}: ${value}`).join(", ")}`);
	...
	receiveSyncLines(data) {
		...
		const openInner = async () => {
			if (connection.connectionMethod == SyncStreamConnectionMethod.HTTP) return await remote.fetchStream(options);
			else return await this.options.remote.socketStreamRaw({
				...options,
		...
		return { async next() {
			if (done) return doneResult;
				...
				if (event.done) {
					...
					command: typeof event.value == "string" ? PowerSyncControlCommand.PROCESS_TEXT_LINE : PowerSyncControlCommand.PROCESS_BSON_LINE,
					payload: event.value
				});
			}
		} };
	...
	async rustSyncIteration(signal, resolvedOptions) {
		...
		if (signal.aborted) throw new AbortOperation("Connection request has been aborted");
		function startCommand() {
			...
			if (resolvedOptions.serializedSchema) options.schema = resolvedOptions.serializedSchema;
		...
		async function invokePowerSyncControl(op, payload) {
			...
			syncImplementation.logger.trace("powersync_control", op, payload == null || typeof payload == "string" ? payload : "<bytes>", rawResponse);
			if (op != PowerSyncControlCommand.STOP) syncImplementation.connectionMayHaveChanged = false;
			return JSON.parse(rawResponse);
		}
		async function handleInstruction(instruction) {
			if ("LogLine" in instruction) switch (instruction.LogLine.severity) {
				case "DEBUG":
			...
			else if ("UpdateSyncStatus" in instruction) syncImplementation.updateSyncStatus(coreStatusToJs(instruction.UpdateSyncStatus.status));
			else if ("FetchCredentials" in instruction) if (instruction.FetchCredentials.did_expire) remote.invalidateCredentials();
			else {
			...
			else if ("DidCompleteSync" in instruction) syncImplementation.updateSyncStatus({ dataFlow: { downloadError: void 0 } });
		}
		try {
			const defaultResult = { immediateRestart: false };
			let controlInvocations = null;
			for (const startInstruction of await startCommand()) if ("EstablishSyncStream" in startInstruction) {
				const syncOptions = {
					path: "/sync/stream",
					abortSignal: signal,
			...
			} else if ("CloseSyncStream" in startInstruction) return defaultResult;
			else await handleInstruction(startInstruction);
			...
			loop: while (true) {
				...
				if (done) break;
				if (!hadSyncLine) {
					if (value.command == PowerSyncControlCommand.PROCESS_TEXT_LINE || value.command == PowerSyncControlCommand.PROCESS_BSON_LINE) {
						hadSyncLine = true;
						this.triggerCrudUpload?.();
					}
				}
				const instructions = await invokePowerSyncControl(value.command, value.payload);
				for (const instruction of instructions) if ("EstablishSyncStream" in instruction) throw new Error("Received EstablishSyncStream while already connected.");
				else if ("CloseSyncStream" in instruction) {
					hideDisconnectOnRestart = instruction.CloseSyncStream.hide_disconnect;
					break loop;
				} else await handleInstruction(instruction);
			}
		} finally {
			this.notifyCompletedUploads = this.handleActiveStreamsChange = void 0;
			notifyTokenRefreshed = void 0;
			await stop();
		}
...
* @internal
*/
var SqliteBucketStorage = class SqliteBucketStorage extends BaseObserver {
	db;
	logger;
	tableNames;
	updateListener;
	...
	constructor(db, logger = Logger.get("SqliteBucketStorage")) {
		...
		this.db = db;
		this.logger = logger;
		this.tableNames = /* @__PURE__ */ new Set();
		this.updateListener = db.registerListener({ tablesUpdated: (update) => {
			if (extractTableUpdates(update).includes(PSInternalTable.CRUD)) this.iterateListeners((l) => l.crudUpdate?.());
		} });
	}
	async init() {
		const existingTableRows = await this.db.getAll(`SELECT name FROM sqlite_master WHERE type='table' AND name GLOB 'ps_data_*'`);
		for (const row of existingTableRows ?? []) this.tableNames.add(row.name);
	}
	async dispose() {
		this.updateListener?.();
	}
	...
	getClientId() {
		if (this._clientId == null) this._clientId = this._getClientId();
	...
	async updateLocalTarget(cb) {
		if (!(await this.db.getAll("SELECT target_op FROM ps_buckets WHERE name = '$local' AND target_op = CAST(? as INTEGER)", ["9223372036854775807"])).length) return false;
		...
		return this.writeTransaction(async (tx) => {
			if ((await tx.execute("SELECT 1 FROM ps_crud LIMIT 1")).rows?.length) {
			...
			if (!rs.rows?.length) throw new Error("SQLite Sequence should not be empty");
			if (rs.rows?.item(0)["seq"] != seqBefore) {
	...
	*/
	async getCrudBatch(limit = 100) {
		...
		const all = [];
		for (const row of crudResult) all.push(CrudEntry.fromRow(row));
		...
		return {
			...
			complete: async (writeCheckpoint) => {
				return this.writeTransaction(async (tx) => {
					...
					if (writeCheckpoint) {
						if ((await tx.execute("SELECT 1 FROM ps_crud LIMIT 1")).rows?.length) await tx.execute("UPDATE ps_buckets SET target_op = CAST(? as INTEGER) WHERE name='$local'", [writeCheckpoint]);
					} else await tx.execute("UPDATE ps_buckets SET target_op = CAST(? as INTEGER) WHERE name='$local'", [this.getMaxOpId()]);
				});
			}
		};
	}
	...
	async hasMigratedSubkeys() {
		const { r } = await this.db.get("SELECT EXISTS(SELECT * FROM ps_kv WHERE key = ?) as r", [SqliteBucketStorage._subkeyMigrationKey]);
		return r != 0;
	}
	async migrateToFixedSubkeys() {
		await this.writeTransaction(async (tx) => {
			await tx.execute("UPDATE ps_oplog SET key = powersync_remove_duplicate_key_encoding(key);");
			await tx.execute("INSERT OR REPLACE INTO ps_kv (key, value) VALUES (?, ?);", [SqliteBucketStorage._subkeyMigrationKey, "1"]);
		});
	}
...
* @internal
*/
var ConnectionClosedError = class ConnectionClosedError extends Error {
...
//#endregion
//#region node_modules/@powersync/web/lib/src/db/sync/WebStreamingSyncImplementation.js
var WebStreamingSyncImplementation = class extends AbstractStreamingSyncImplementation {
	constructor(options) {
		super(options);
	}
	get webOptions() {
	...
	async obtainLock(lockOptions) {
		...
		if (lockOptions.type == LockType.SYNC) this.logger.debug("requesting lock for ", identifier);
		return getNavigatorLocks().request(identifier, { signal: lockOptions.signal }, lockOptions.callback);
	}
};
//#endregion
//#region node_modules/@powersync/web/lib/src/worker/sync/BroadcastLogger.js
/**
* Broadcasts logs to all clients
*/
var BroadcastLogger = class {
	clients;
	TRACE;
	DEBUG;
	INFO;
...
//#endregion
//#region node_modules/@powersync/web/lib/src/shared/tab_close_signal.js
/**
...
function generateTabCloseSignal(abort) {
	return new Promise((resolve, reject) => {
		...
		getNavigatorLocks().request(`tab-close-signal-${crypto.randomUUID()}`, options, (lock) => {
			resolve(lock.name);
			return new Promise((resolve) => {
				if (abort) abort.addEventListener("abort", () => resolve());
			});
		}).catch(reject);
	});
}
//#endregion
//#region node_modules/@powersync/web/lib/src/worker/sync/SharedSyncImplementation.js
/**
* @internal
...
*/
var SharedSyncImplementation = class extends BaseObserver {
	ports;
	isInitialized;
	statusListener;
	...
	constructor() {
		...
		this.isInitialized = new Promise((resolve) => {
			const callback = this.registerListener({ initialized: () => {
				resolve();
				callback?.();
			} });
		});
		...
		this.connectionManager = new ConnectionManager({
			createSyncImplementation: async () => {
				...
				return {
					sync,
					onDispose: sync.registerListener({ statusChanged: (status) => {
	...
	*/
	async getLastWrappedPort() {
		return await this.portMutex.runExclusive(() => {
			for (let i = this.ports.length - 1; i >= 0; i--) if (!this.ports[i].isClosing) return this.ports[i];
		});
	}
	/**
	...
	*/
	...
	collectActiveSubscriptions() {
		...
		for (const port of this.ports) for (const stream of port.currentSubscriptions) {
	...
	*/
	async setParams(params) {
		...
		if (this.syncParams) return;
		this.syncParams = params;
		if (params.streamOptions?.flags?.broadcastLogs) this.logger = this.broadCastLogger;
	...
	*/
	async addPort(port) {
		return await this.portMutex.runExclusive(() => {
			...
			if (status) portProvider.clientProvider.statusChanged(status.toJSON());
			return portProvider;
		});
	}
	/**
	...
	*/
		...
		return await this.portMutex.runExclusive(async () => {
			...
			*/
			[this.fetchCredentialsController, this.uploadDataController].forEach((abortController) => {
				if (abortController?.activePort == port) abortController.controller.abort(new AbortOperation("Closing pending requests after client port is removed"));
			});
			for (const closeListener of trackedPort.closeListeners) await closeListener();
			this.collectActiveSubscriptions();
			return () => trackedPort.clientProvider[releaseProxy]();
		});
	}
	triggerCrudUpload() {
	...
	async withSyncImplementation(callback) {
		await this.waitForReady();
		if (this.connectionManager.syncStreamImplementation) return callback(this.connectionManager.syncStreamImplementation);
		return callback(await new Promise((resolve) => {
			const dispose = this.connectionManager.registerListener({ syncStreamCreated: (sync) => {
				resolve(sync);
				dispose?.();
			} });
		}));
		...
		return new WebStreamingSyncImplementation({
			...
			remote: new WebRemote({
				invalidateCredentials: async () => {
					...
					if (!lastPort) throw new Error("No client port found to invalidate credentials");
					try {
				...
				fetchCredentials: async () => {
					const lastPort = await this.getLastWrappedPort();
					if (!lastPort) throw new Error("No client port found to fetch credentials");
			...
			uploadCrud: async () => {
				const lastPort = await this.getLastWrappedPort();
				if (!lastPort) throw new Error("No client port found to upload crud");
				return new Promise(async (resolve, reject) => {
					const abortController = new AbortController();
	...
	*/
	async openInternalDB(handleClosed) {
		...
		if (!client) throw new Error(`Could not open DB connection since no client is connected.`);
		...
		*/
		...
		const removeCloseListener = () => {
			...
			if (index >= 0) client.closeListeners.splice(index, 1);
		};
		client.closeListeners.push(closeListener);
		const remote = wrap(await withAbort({
		...
		const db = await withAbort({
			action: async () => {
				return new DatabaseClient({
					connection: await remote.connectToExisting({
						identifier,
						lockName: clientLockName
					}),
					source: remote,
	...
	generateReconnectableDatabase() {
		...
		class ReconnectPool extends BaseObserver {
				...
				if (this.connectionState == null) {
					...
					this.connectionState = (async () => {
						try {
							...
							db.registerListener({ tablesUpdated: (notification) => {
			...
			async close() {
				if (this.connectionState != null) await (await this.connectionState).close();
			}
			...
			async readLock(fn, options) {
				return (await this.connect()).readLock(fn, options);
			}
			async writeLock(fn, options) {
				return (await this.connect()).writeLock(fn, options);
			}
			async refreshSchema() {}
		}
		return new (DBAdapterDefaultMixin(ReconnectPool))();
	}
	/**
...
function withAbort(options) {
	...
	return new Promise((resolve, reject) => {
		...
		action().then((data) => {
			if (signal.aborted) return completePromise(() => cleanupOnAbort?.(data));
			completePromise(() => resolve(data));
		}).catch((e) => completePromise(() => reject(e)));
	});
}
//#endregion
//#region node_modules/@powersync/web/lib/src/worker/sync/WorkerClient.js
/**
...
var WorkerClient = class {
	...
	constructor(sync, port) {
		...
		*/
		this.port.addEventListener("message", async (event) => {
			if (event.data?.event == SharedSyncClientEvent.CLOSE_CLIENT) await this.removePort();
		});
	}
	async removePort() {
	...
	*/
	...
	updateSubscriptions(subscriptions) {
		if (this.resolvedPort) this.sync.updateSubscriptions(this.resolvedPort, subscriptions);
	}
	disconnect() {
...
//#endregion
//#region node_modules/@powersync/web/lib/src/worker/sync/SharedSyncImplementation.worker.js
const _self = self;
...
//# sourceMappingURL=SharedSyncImplementation.worker-1OJHCCw2.js.map