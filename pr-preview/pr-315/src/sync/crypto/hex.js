...
var bytesToHex = (bytes) => {
	let out = "";
	for (const byte of bytes) out += HEX_ALPHABET[byte >> 4] + HEX_ALPHABET[byte & 15];
	return out;
};
...
var hexToBytes = (hex) => {
	...
	for (let i = 0; i < out.length; i++) {
		...
		if (!/^[0-9a-fA-F]{2}$/.test(hex.slice(i * 2, i * 2 + 2))) throw new Error(`hexToBytes: non-hex characters at offset ${i * 2}`);
		out[i] = byte;
	}
	return out;
};
