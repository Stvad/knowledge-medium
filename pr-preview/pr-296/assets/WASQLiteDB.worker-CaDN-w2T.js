...
async function asyncModuleFactory(encryptionKey) {
	if (encryptionKey) {
		const { default: factory } = await import("./mc-wa-sqlite-async-DjC0-8tr.js");
		return factory();
	} else {
		const { default: factory } = await import("./wa-sqlite-async-D_tTf3dB.js");
		return factory();
	}
}
async function syncModuleFactory(encryptionKey) {
	if (encryptionKey) {
		const { default: factory } = await import("./mc-wa-sqlite-DqoBHEL-.js");
		return factory();
	} else {
		const { default: factory } = await import("./wa-sqlite-BIEjRZ-1.js");
		return factory();
	}
}
/**
...
const DEFAULT_MODULE_FACTORIES = {
	[WASQLiteVFS.IDBBatchAtomicVFS]: async (options) => {
		...
		const { IDBBatchAtomicVFS } = await import("./IDBBatchAtomicVFS-VtxhKB7Z.js");
		return {
			module,
	...
	[WASQLiteVFS.AccessHandlePoolVFS]: async (options) => {
		...
		const { AccessHandlePoolVFS } = await import("./AccessHandlePoolVFS-CEZwz_T_.js");
		return {
			module,
	...
	[WASQLiteVFS.OPFSCoopSyncVFS]: async (options) => {
		const module = await syncModuleFactory(options.encryptionKey);
		const { OPFSCoopSyncVFS } = await import("./OPFSCoopSyncVFS-D1jCtqc0.js");
		return {
			module,
	...
	[WASQLiteVFS.OPFSWriteAheadVFS]: async (options) => {
		const module = await syncModuleFactory(options.encryptionKey);
		const { OPFSWriteAheadVFS } = await import("./OPFSWriteAheadVFS-DzUQRNbD.js");
		return {
			module,
...
//# sourceMappingURL=WASQLiteDB.worker-CaDN-w2T.js.map