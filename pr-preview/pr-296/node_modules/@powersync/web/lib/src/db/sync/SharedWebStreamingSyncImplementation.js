import { expose, releaseProxy, transfer, wrap } from "../../../../../../comlink/dist/esm/comlink.js";
...
//#region node_modules/@powersync/web/lib/src/db/sync/SharedWebStreamingSyncImplementation.js
/**
* The shared worker will trigger methods on this side of the message port
* via this client provider.
*/
var SharedSyncClientProvider = class extends AbstractSharedSyncClientProvider {
	options;
	statusChanged;
	webDB;
...
*/
var SharedWebStreamingSyncImplementation = class extends WebStreamingSyncImplementation {
	syncManager;
	clientProvider;
	messagePort;
	isInitialized;
	...
	constructor(options) {
		...
		*/
		...
		if (syncWorker) if (typeof syncWorker === "function") this.messagePort = syncWorker(resolvedWorkerOptions).port;
		else this.messagePort = new SharedWorker(`${syncWorker}`, { 
		/* @vite-ignore */
name: `shared-sync-${this.webOptions.identifier}` }).port;
		else this.messagePort = new SharedWorker(new URL(
			/* @vite-ignore */
			"/knowledge-medium/pr-preview/pr-296/assets/SharedSyncImplementation.worker-1OJHCCw2.js",
			"" + import.meta.url
		), {
			/* @vite-ignore */
		...
		*/
		...
		this.syncManager = wrap(this.messagePort);
		/**
		...
		*/
		expose(this.clientProvider, this.messagePort);
	...
	async dispose() {
		...
		await new Promise((resolve) => {
			this.messagePort.addEventListener("message", (event) => {
				if (event.data?.event === SharedSyncClientEvent.CLOSE_ACK) resolve();
			});
...
//#endregion
export { SharedWebStreamingSyncImplementation };
