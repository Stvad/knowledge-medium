...
${i(n,8)}
    ...
    `,params:n.map(e=>e===`id`?`Id`:{Column:e})},delete:{sql:`DELETE FROM blocks_synced WHERE id = ?`,params:[`Id`]}},v=[{key:`id`,sqlExpression:e=>`${e}.id`},{key:`workspaceId`,sqlExpression:e=>`${e}.workspace_id`},{key:`parentId`,sqlExpression:e=>`${e}.parent_id`},{key:`referenceTargetId`,sqlExpression:e=>`${e}.reference_target_id`},{key:`isFieldForm`,sqlExpression:e=>`json(CASE WHEN ${e}.is_field_form = 1 THEN 'true' ELSE 'false' END)`},{key:`orderKey`,sqlExpression:e=>`${e}.order_key`},{key:`content`,sqlExpression:e=>`${e}.content`},{key:`properties`,sqlExpression:e=>`json(${e}.properties_json)`},{key:`references`,sqlExpression:e=>`json(${e}.references_json)`},{key:`createdAt`,sqlExpression:e=>`${e}.created_at`},{key:`updatedAt`,sqlExpression:e=>`${e}.updated_at`},{key:`userUpdatedAt`,sqlExpression:e=>`coalesce(${e}.user_updated_at, ${e}.updated_at)`},{key:`createdBy`,sqlExpression:e=>`${e}.created_by`},{key:`updatedBy`,sqlExpression:e=>`${e}.updated_by`},{key:`deleted`,sqlExpression:e=>`json(CASE WHEN ${e}.deleted THEN 'true' ELSE 'false' END)`}],y=e=>`
  json_object(
${i(v.map(t=>`'${t.key}', ${t.sqlExpression(e)}`),4)}
  )
`,b=(e,t)=>{if(!e)return t;try{return JSON.parse(e)}catch(e){return console.warn(`Failed to parse stored block JSON`,e),t}},x=e=>e?b(e,null)??void 0:void 0,S=e=>({id:e.id,workspaceId:e.workspace_id,parentId:e.parent_id,referenceTargetId:e.reference_target_id??null,isFieldForm:e.is_field_form===1,orderKey:e.order_key,content:e.content,properties:b(e.properties_json,{}),references:b(e.references_json,[]),createdAt:e.created_at,updatedAt:e.updated_at,userUpdatedAt:e.user_updated_at??e.updated_at,createdBy:e.created_by,updatedBy:e.updated_by,deleted:!!e.deleted}),C=e=>[e.id,e.workspaceId,e.parentId,e.orderKey,e.content,JSON.stringify(e.properties??{}),JSON.stringify(e.references??[]),e.createdAt,e.updatedAt,e.userUpdatedAt,e.createdBy,e.updatedBy,+!!e.deleted,e.referenceTargetId??null,e.isFieldForm?1:null],w=t=>C(t).slice(0,e.length);export{_ as BLOCKS_SYNCED_RAW_TABLE,r as BLOCKS_TABLE_COLUMN_NAMES,t as BLOCK_LOCAL_COLUMNS,e as BLOCK_STORAGE_COLUMNS,h as CREATE_BLOCKS_ANY_FIELD_FORM_INDEX_SQL,m as CREATE_BLOCKS_FIELD_FORM_INDEX_SQL,u as CREATE_BLOCKS_PARENT_ORDER_INDEX_SQL,p as CREATE_BLOCKS_REFERENCE_TARGET_PARENT_INDEX_SQL,l as CREATE_BLOCKS_SYNCED_TABLE_SQL,s as CREATE_BLOCKS_TABLE_SQL,d as CREATE_BLOCKS_WORKSPACE_ACTIVE_INDEX_SQL,f as CREATE_BLOCKS_WORKSPACE_NONEMPTY_PROPERTIES_INDEX_SQL,g as DROP_STALE_ANY_FIELD_FORM_INDEX_SQL,a as SELECT_BLOCK_COLUMNS_SQL,C as blockToRowParams,w as blockToSyncedRowParams,y as buildBlockSnapshotJsonSql,o as buildQualifiedBlockColumnsSql,c as ensureBlockLocalColumns,S as parseBlockRow,x as parseBlockSnapshotJson};
