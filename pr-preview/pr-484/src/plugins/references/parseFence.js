import{parseReferences as e}from"./referenceParser.js";var t=`
  SELECT blocks_fts.block_id AS sourceId, b.content AS content
  FROM blocks_fts
  JOIN blocks b
    ON b.id = blocks_fts.block_id
   AND b.workspace_id = blocks_fts.workspace_id
  WHERE blocks_fts.workspace_id = ?
    AND blocks_fts MATCH ?
    AND b.deleted = 0
    AND b.content != ''
  ORDER BY b.order_key, b.id
`,n=`
  SELECT b.id AS sourceId, b.content AS content
  FROM blocks b
  WHERE b.workspace_id = ?
    AND instr(b.content, ?) > 0
    AND b.deleted = 0
    AND b.content != ''
  ORDER BY b.order_key, b.id
`,r=e=>`"${e.replace(/"/g,`""`)}"`,i=e=>e.includes(`\0`),a=async(a,o,s)=>{if(s===``)return[];let c=`[[${s}]]`;return(await a.getAll(i(c)?n:t,[o,i(c)?c:r(c)])).filter(t=>e(t.content).some(e=>e.alias===s))},o=(...e)=>{let t=new Map;for(let n of e)for(let e of n)t.has(e.sourceId)||t.set(e.sourceId,e);return[...t.values()]};export{o as mergeReferrers,a as wikilinkSourcesByContent};
//# sourceMappingURL=parseFence.js.map