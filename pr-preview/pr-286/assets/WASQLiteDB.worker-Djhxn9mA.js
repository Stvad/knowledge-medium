...
async function asyncModuleFactory(encryptionKey) {
	if (encryptionKey) {
		const { default: factory } = await import("./mc-wa-sqlite-async-PfXJpjMg.js");
		return factory();
	} else {
		const { default: factory } = await import("./wa-sqlite-async-rwmpdyiA.js");
		return factory();
	}
}
async function syncModuleFactory(encryptionKey) {
	if (encryptionKey) {
		const { default: factory } = await import("./mc-wa-sqlite-Dn_o7hSN.js");
		return factory();
	} else {
		const { default: factory } = await import("./wa-sqlite-BW4ZbwKz.js");
		return factory();
	}
}
/**
...
const DEFAULT_MODULE_FACTORIES = {
	[WASQLiteVFS.IDBBatchAtomicVFS]: async (options) => {
		...
		const { IDBBatchAtomicVFS } = await import("./IDBBatchAtomicVFS-B8sCib55.js");
		return {
			module,
	...
	[WASQLiteVFS.AccessHandlePoolVFS]: async (options) => {
		...
		const { AccessHandlePoolVFS } = await import("./AccessHandlePoolVFS-I4u2L2mx.js");
		return {
			module,
	...
	[WASQLiteVFS.OPFSCoopSyncVFS]: async (options) => {
		const module = await syncModuleFactory(options.encryptionKey);
		const { OPFSCoopSyncVFS } = await import("./OPFSCoopSyncVFS--UcCjcOt.js");
		return {
			module,
	...
	[WASQLiteVFS.OPFSWriteAheadVFS]: async (options) => {
		const module = await syncModuleFactory(options.encryptionKey);
		const { OPFSWriteAheadVFS } = await import("./OPFSWriteAheadVFS-DW2UbeBE.js");
		return {
			module,
...
//# sourceMappingURL=WASQLiteDB.worker-Djhxn9mA.js.map