...
import { dailyNoteDateValue } from "./dailyNotes.js";
//#region src/plugins/daily-notes/backfill.ts
/**
...
*  alias). */
var SELECT_LEGACY_DAILY_NOTES_MISSING_DATE_SQL = `
...
var BATCH_SIZE = 500;
var dailyNoteDateBackfill = {
	id: "daily-note-date-from-alias-v2",
	run: async ({ workspaceId, getAll, tx }) => {
		const rows = await getAll(SELECT_LEGACY_DAILY_NOTES_MISSING_DATE_SQL, [workspaceId]);
		for (let i = 0; i < rows.length; i += BATCH_SIZE) {
			...
			await tx(async (t) => {
				for (const { id, iso } of batch) {
					...
					if (block.properties[dailyNoteDateProp.name] !== void 0) continue;
