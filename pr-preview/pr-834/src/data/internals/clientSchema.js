...
`,we=`PRAGMA optimize(0x03)`,Te=`PRAGMA optimize`,$=`analyze_unbounded_stats_v1`,Ee=`
  SELECT 1 FROM client_schema_state WHERE key = '${$}'
`,De=`
  INSERT OR REPLACE INTO client_schema_state (key, completed_at)
  VALUES ('${$}', strftime('%s', 'now') * 1000)
`,Oe=`SELECT COUNT(*) AS count FROM blocks`,ke=[`SELECT id FROM blocks WHERE workspace_id = '' AND deleted = 0`,`SELECT target_id FROM block_references WHERE workspace_id = '' AND alias = ''`,`SELECT block_id FROM block_types WHERE type = '' AND workspace_id = ''`,`SELECT block_id FROM block_aliases WHERE workspace_id = '' AND alias = ''`],Ae=`reproject_ref:`,je=`
...
`,ze=`
  INSERT OR REPLACE INTO client_schema_state (key, completed_at)
  VALUES (?, strftime('%s', 'now') * 1000)
`,Be=/^\s*CREATE\s+TRIGGER\s+(?:IF\s+NOT\s+EXISTS\s+)?([A-Za-z0-9_]+)/i,Ve=e=>e.flatMap(e=>{let t=e.match(Be)?.[1];return t?[`DROP TRIGGER IF EXISTS ${t}`,e]:[e]}),He=async(e,t,n,r)=>{await e.execute(`DROP TRIGGER IF EXISTS ${t}`);try{await r()}finally{await e.execute(n)}},Ue=Ve([a,o,s,c,l,u,ee,te,ne,re,ie,oe,p,m,h,g,ae,_,v,y,b,x,d,f,k,A,j,P,F,I,L,R,z,B,V,H,U,W,G,K,ue,de,J,S,C]),We=[`blocks_row_event_insert`,`blocks_row_event_update`,`blocks_row_event_delete`,`blocks_upload_insert`,`blocks_upload_update`,`blocks_parent_workspace_check_insert`,`blocks_parent_workspace_check_update`,`blocks_parent_not_deleted_check_insert`,`blocks_parent_not_deleted_check_update`,`blocks_alias_insert`,`blocks_alias_update`,`blocks_alias_delete`,`block_aliases_workspace_alias_unique`,`blocks_type_insert`,`blocks_type_update`,`blocks_type_delete`,`blocks_fts_insert`,`blocks_fts_update`,`blocks_fts_delete`,`blocks_synced_changes_insert`,`blocks_synced_changes_delete`],Ge=async e=>{await e.getOptional(fe)===null&&(await e.execute(me),await e.execute(pe))},Ke=async e=>{await e.getOptional(he)===null&&(await e.execute(_e),await e.execute(ge))},qe=async e=>{await e.getOptional(be)===null&&(await e.execute(Se),await e.execute(Ce),await e.execute(xe))},Je=async e=>{let t=!1;for(let n of[`blocks`,`blocks_synced`])(await e.getAll(`PRAGMA table_info(${n})`)).some(e=>e.name===`user_updated_at`)||(await e.execute(`ALTER TABLE ${n} ADD COLUMN user_updated_at INTEGER`),n===`blocks`&&(t=!0));t&&await He(e,`blocks_row_event_update`,A,async()=>{await e.execute(`UPDATE blocks SET user_updated_at = updated_at`)})},Ye=async n=>{let r=await n.getAll(`PRAGMA table_info(blocks_synced)`);if(r.length!==0){for(let t of e)r.some(e=>e.name===t.name)||await n.execute(`ALTER TABLE blocks_synced ADD COLUMN ${t.definition}`);await n.getOptional(ve)===null&&(await n.execute(t),await n.execute(ye))}},Xe=async e=>{for(let t of[`tx_context`,`row_events`]){let n=await e.getAll(`PRAGMA table_info(${t})`);n.length!==0&&(n.some(e=>e.name===`group_id`)||await e.execute(`ALTER TABLE ${t} ADD COLUMN group_id TEXT`))}},Ze=async e=>(await e.getOptional(Oe))?.count??0,Qe=async e=>{for(let t of ke)try{await e.execute(`EXPLAIN QUERY PLAN ${t}`)}catch(e){console.warn(`[clientSchema] ANALYZE arming probe failed:`,t,e)}},$e=e=>{let t=e?.rows?._array;return Array.isArray(t)?t.map(e=>e.optimize).filter(e=>typeof e==`string`):null},et=Promise.resolve(),tt=e=>{let t=et.then(e,e);return et=t,t},nt=async e=>tt(async()=>{await Qe(e);let t=$e(await e.execute(we));return await e.execute(Te),await rt(e),{proposed:t}}),rt=async e=>{await e.getOptional(Ee)===null&&(await e.execute(`ANALYZE`),await e.execute(De))},it=async e=>tt(async()=>{let t=await Ze(e);return await e.execute(`ANALYZE`),{count:t}});export{Y as ALIAS_BACKFILL_MARKER_KEY,ke as ANALYZE_ARMING_PROBES,we as ANALYZE_DRY_RUN_SQL,Te as ANALYZE_OPTIMIZE_SQL,Se as BACKFILL_BLOCKS_FTS_ROWIDS_SQL,Ce as BACKFILL_BLOCKS_FTS_SQL,me as BACKFILL_BLOCK_ALIASES_SQL,_e as BACKFILL_BLOCK_TYPES_SQL,Z as BLOCKS_FTS_BACKFILL_MARKER_KEY,X as BLOCK_TYPES_BACKFILL_MARKER_KEY,Ne as CLEAR_REPROJECT_REF_MARKER_SQL,Ue as CLIENT_SCHEMA_STATEMENTS,We as CLIENT_SCHEMA_TRIGGER_NAMES,H as CREATE_BLOCKS_ALIAS_DELETE_TRIGGER_SQL,B as CREATE_BLOCKS_ALIAS_INSERT_TRIGGER_SQL,V as CREATE_BLOCKS_ALIAS_UPDATE_TRIGGER_SQL,j as CREATE_BLOCKS_DELETE_ROW_EVENT_TRIGGER_SQL,J as CREATE_BLOCKS_FTS_DELETE_TRIGGER_SQL,ue as CREATE_BLOCKS_FTS_INSERT_TRIGGER_SQL,h as CREATE_BLOCKS_FTS_ROWIDS_TABLE_SQL,g as CREATE_BLOCKS_FTS_TABLE_SQL,de as CREATE_BLOCKS_FTS_UPDATE_TRIGGER_SQL,k as CREATE_BLOCKS_INSERT_ROW_EVENT_TRIGGER_SQL,R as CREATE_BLOCKS_PARENT_NOT_DELETED_INSERT_TRIGGER_SQL,z as CREATE_BLOCKS_PARENT_NOT_DELETED_UPDATE_TRIGGER_SQL,C as CREATE_BLOCKS_SYNCED_CHANGES_DELETE_TRIGGER_SQL,x as CREATE_BLOCKS_SYNCED_CHANGES_ID_OP_INDEX_SQL,S as CREATE_BLOCKS_SYNCED_CHANGES_INSERT_TRIGGER_SQL,b as CREATE_BLOCKS_SYNCED_CHANGES_TABLE_SQL,K as CREATE_BLOCKS_TYPE_DELETE_TRIGGER_SQL,W as CREATE_BLOCKS_TYPE_INSERT_TRIGGER_SQL,G as CREATE_BLOCKS_TYPE_UPDATE_TRIGGER_SQL,A as CREATE_BLOCKS_UPDATE_ROW_EVENT_TRIGGER_SQL,P as CREATE_BLOCKS_UPLOAD_INSERT_TRIGGER_SQL,F as CREATE_BLOCKS_UPLOAD_UPDATE_TRIGGER_SQL,I as CREATE_BLOCKS_WORKSPACE_INVARIANT_INSERT_TRIGGER_SQL,L as CREATE_BLOCKS_WORKSPACE_INVARIANT_UPDATE_TRIGGER_SQL,re as CREATE_BLOCK_ALIASES_TABLE_SQL,U as CREATE_BLOCK_ALIASES_WORKSPACE_UNIQUE_TRIGGER_SQL,ie as CREATE_BLOCK_ALIASES_WS_ALIAS_INDEX_SQL,oe as CREATE_BLOCK_ALIASES_WS_ALIAS_LOWER_INDEX_SQL,p as CREATE_BLOCK_TYPES_TABLE_SQL,m as CREATE_BLOCK_TYPES_TYPE_WORKSPACE_INDEX_SQL,ae as CREATE_CLIENT_SCHEMA_STATE_TABLE_SQL,te as CREATE_COMMAND_EVENTS_CREATED_INDEX_SQL,ee as CREATE_COMMAND_EVENTS_TABLE_SQL,ne as CREATE_COMMAND_EVENTS_WORKSPACE_INDEX_SQL,v as CREATE_PS_CRUD_REJECTED_REJECTED_AT_INDEX_SQL,_ as CREATE_PS_CRUD_REJECTED_TABLE_SQL,y as CREATE_PS_CRUD_REJECTED_TX_ID_INDEX_SQL,l as CREATE_ROW_EVENTS_BLOCK_INDEX_SQL,u as CREATE_ROW_EVENTS_CREATED_INDEX_SQL,s as CREATE_ROW_EVENTS_TABLE_SQL,c as CREATE_ROW_EVENTS_TX_INDEX_SQL,a as CREATE_TX_CONTEXT_TABLE_SQL,f as DROP_ANALYZE_INDEX_SET_MARKERS_SQL,d as DROP_BLOCKS_WORKSPACE_TYPE_INDEX_SQL,Le as RECONCILE_RESCAN_MARKER_PREFIX,xe as RECORD_BLOCKS_FTS_BACKFILL_DONE_SQL,pe as RECORD_BLOCK_ALIASES_BACKFILL_DONE_SQL,ge as RECORD_BLOCK_TYPES_BACKFILL_DONE_SQL,ze as RECORD_RECONCILE_RESCAN_MARKER_SQL,Me as RECORD_REPROJECT_REF_MARKER_SQL,ye as RECORD_STAGING_NEEDS_APPLY_SEEDED_SQL,De as RECORD_UNBOUNDED_ANALYZE_DONE_SQL,Ie as RECORD_WORKSPACE_BACKFILL_MARKER_SQL,Ae as REPROJECT_REF_MARKER_PREFIX,o as SEED_TX_CONTEXT_ROW_SQL,Oe as SELECT_BLOCKS_COUNT_SQL,be as SELECT_BLOCKS_FTS_BACKFILL_DONE_SQL,fe as SELECT_BLOCK_ALIASES_BACKFILL_DONE_SQL,he as SELECT_BLOCK_TYPES_BACKFILL_DONE_SQL,Re as SELECT_RECONCILE_RESCAN_MARKER_SQL,je as SELECT_REPROJECT_REF_MARKERS_SQL,ve as SELECT_STAGING_NEEDS_APPLY_SEEDED_SQL,Ee as SELECT_UNBOUNDED_ANALYZE_DONE_SQL,Fe as SELECT_WORKSPACE_BACKFILL_MARKERS_SQL,Q as STAGING_NEEDS_APPLY_SEEDED_MARKER_KEY,$ as UNBOUNDED_ANALYZE_MARKER_KEY,Pe as WORKSPACE_BACKFILL_MARKER_PREFIX,Ge as backfillBlockAliasesIfEmpty,Ke as backfillBlockTypesIfEmpty,qe as backfillBlocksFtsIfEmpty,Je as ensureBlockUserUpdatedAtColumn,Ye as ensureStagingNeedsApplyColumn,Xe as ensureUndoGroupIdColumns,Ze as getBlocksCount,nt as runAnalyzeIfStale,it as runAnalyzeNow};
