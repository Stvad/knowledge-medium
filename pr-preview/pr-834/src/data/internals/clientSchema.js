...
`.trim(),S=e=>`
      ...
      END
`.trim(),C=S(`tx_id`),w=`COALESCE((SELECT source FROM tx_context WHERE id = 1), 'sync')`,T=S(`group_id`),E=`
...
`,O=`
  ...
  END
`,k=[{name:`workspace_id`,jsonValue:e=>`${e}.workspace_id`},{name:`parent_id`,jsonValue:e=>`${e}.parent_id`},{name:`order_key`,jsonValue:e=>`${e}.order_key`},{name:`content`,jsonValue:e=>`${e}.content`},{name:`properties_json`,jsonValue:e=>`${e}.properties_json`},{name:`references_json`,jsonValue:e=>`${e}.references_json`},{name:`created_at`,jsonValue:e=>`${e}.created_at`},{name:`updated_at`,jsonValue:e=>`${e}.updated_at`},{name:`user_updated_at`,jsonValue:e=>`${e}.user_updated_at`},{name:`created_by`,jsonValue:e=>`${e}.created_by`},{name:`updated_by`,jsonValue:e=>`${e}.updated_by`},{name:`deleted`,jsonValue:e=>`json(CASE WHEN ${e}.deleted THEN 'true' ELSE 'false' END)`}],ue=e=>`
      json_object(
${k.map(t=>`        '${t.name}', ${t.jsonValue(e)}`).join(`,
`)}
      )
`.trim(),de=k.map(e=>`OLD.${e.name} IS NOT NEW.${e.name}`).join(`
    OR `),fe=()=>`
      json_remove(
        json_set(
          '{}',
...
${k.filter(e=>e.name!==`workspace_id`).map(e=>`          CASE WHEN OLD.${e.name} IS NOT NEW.${e.name} THEN '$.${e.name}' ELSE '$.__noop' END, ${e.jsonValue(`NEW`)}`).join(`,
`)}
        ),
        '$.__noop'
      )
`.trim(),A=`(SELECT tx_seq FROM tx_context WHERE id = 1)`,j=`
...
`,G=`
  ...
  END
`,K=`block_aliases_backfill_v1`,he=`
  SELECT 1 FROM client_schema_state WHERE key = '${K}'
`,ge=`
  INSERT OR REPLACE INTO client_schema_state (key, completed_at)
  VALUES ('${K}', strftime('%s', 'now') * 1000)
`,_e=`
  INSERT OR IGNORE INTO block_aliases (block_id, workspace_id, alias, alias_lower)
...
`,be=`
  ...
  WHERE b.deleted = 0 AND typeof(je.value) = 'text'
`,J=`blocks_fts_backfill_v1`,Y=`staging_needs_apply_seeded`,xe=`
...
`,De=`PRAGMA optimize(0x03)`,Oe=`PRAGMA optimize(0x02)`,ke=`PRAGMA analysis_limit=0`,X=`analyze_unbounded_stats_v1`,Ae=[`blocks`,`block_references`,`block_types`,`block_aliases`],je=`
  SELECT 1 FROM client_schema_state WHERE key = '${X}'
`,Z=`
  INSERT OR REPLACE INTO client_schema_state (key, completed_at)
  VALUES ('${X}', strftime('%s', 'now') * 1000)
`,Me=`SELECT COUNT(*) AS count FROM blocks`,Ne=[`SELECT id FROM blocks WHERE workspace_id = '' AND deleted = 0`,`SELECT target_id FROM block_references WHERE workspace_id = '' AND alias = ''`,`SELECT block_id FROM block_types WHERE type = '' AND workspace_id = ''`,`SELECT block_id FROM block_aliases WHERE workspace_id = '' AND alias = ''`],Pe=`reproject_ref:`,Fe=`
...
`,Ue=`
  INSERT OR REPLACE INTO client_schema_state (key, completed_at)
  VALUES (?, strftime('%s', 'now') * 1000)
`,We=/^\s*CREATE\s+TRIGGER\s+(?:IF\s+NOT\s+EXISTS\s+)?([A-Za-z0-9_]+)/i,Ge=e=>e.flatMap(e=>{let t=e.match(We)?.[1];return t?[`DROP TRIGGER IF EXISTS ${t}`,e]:[e]}),Ke=async(e,t,n,r)=>{await e.execute(`DROP TRIGGER IF EXISTS ${t}`);try{await r()}finally{await e.execute(n)}},qe=Ge([a,o,s,c,l,u,ee,te,ne,re,ie,oe,d,f,p,m,ae,le,h,g,_,v,se,ce,E,D,O,j,M,N,P,F,I,L,R,z,B,V,H,U,pe,me,G,y,b]),Je=[`blocks_row_event_insert`,`blocks_row_event_update`,`blocks_row_event_delete`,`blocks_upload_insert`,`blocks_upload_update`,`blocks_parent_workspace_check_insert`,`blocks_parent_workspace_check_update`,`blocks_parent_not_deleted_check_insert`,`blocks_parent_not_deleted_check_update`,`blocks_alias_insert`,`blocks_alias_update`,`blocks_alias_delete`,`block_aliases_workspace_alias_unique`,`blocks_type_insert`,`blocks_type_update`,`blocks_type_delete`,`blocks_fts_insert`,`blocks_fts_update`,`blocks_fts_delete`,`blocks_synced_changes_insert`,`blocks_synced_changes_delete`],Ye=async e=>{await e.getOptional(he)===null&&(await e.execute(_e),await e.execute(ge))},Xe=async e=>{await e.getOptional(ve)===null&&(await e.execute(be),await e.execute(ye))},Ze=async e=>{await e.getOptional(Ce)===null&&(await e.execute(Te),await e.execute(Ee),await e.execute(we))},Qe=async e=>{let t=!1;for(let n of[`blocks`,`blocks_synced`])(await e.getAll(`PRAGMA table_info(${n})`)).some(e=>e.name===`user_updated_at`)||(await e.execute(`ALTER TABLE ${n} ADD COLUMN user_updated_at INTEGER`),n===`blocks`&&(t=!0));t&&await Ke(e,`blocks_row_event_update`,D,async()=>{await e.execute(`UPDATE blocks SET user_updated_at = updated_at`)})},$e=async n=>{let r=await n.getAll(`PRAGMA table_info(blocks_synced)`);if(r.length!==0){for(let t of e)r.some(e=>e.name===t.name)||await n.execute(`ALTER TABLE blocks_synced ADD COLUMN ${t.definition}`);await n.getOptional(xe)===null&&(await n.execute(t),await n.execute(Se))}},et=async e=>{for(let t of[`tx_context`,`row_events`]){let n=await e.getAll(`PRAGMA table_info(${t})`);n.length!==0&&(n.some(e=>e.name===`group_id`)||await e.execute(`ALTER TABLE ${t} ADD COLUMN group_id TEXT`))}},tt=async e=>(await e.getOptional(Me))?.count??0,nt=async e=>{for(let t of Ne)try{await e.execute(`EXPLAIN QUERY PLAN ${t}`)}catch(e){console.warn(`[clientSchema] ANALYZE arming probe failed:`,t,e)}},rt=e=>{let t=e?.rows?._array;return Array.isArray(t)?t.map(e=>e.optimize).filter(e=>typeof e==`string`):null},it=Promise.resolve(),Q=e=>{let t=it.then(e,e);return it=t,t},at=async e=>Q(async()=>{await nt(e);let t=rt(await e.execute(De));return await $(e,Oe),{proposed:t}}),$=async(e,t)=>{await e.execute(ke),await e.execute(t)},ot=async e=>Q(async()=>{if(await e.getOptional(je)!==null)return!1;let t=!0;for(let n of Ae)if(await st(e,n))try{await $(e,`ANALYZE ${n}`)}catch(e){console.warn(`[clientSchema] exact-stats repair failed for ${n}`,e),t=!1}return t?(await e.execute(Z),!0):!1}),st=async(e,t)=>await e.getOptional(`SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = '${t}'`)!==null,ct=async e=>Q(async()=>{let t=await tt(e);return await $(e,`ANALYZE`),await e.execute(Z),{count:t}});export{K as ALIAS_BACKFILL_MARKER_KEY,Ne as ANALYZE_ARMING_PROBES,De as ANALYZE_DRY_RUN_SQL,Oe as ANALYZE_OPTIMIZE_SQL,Ae as ANALYZE_REPAIR_TABLES,Te as BACKFILL_BLOCKS_FTS_ROWIDS_SQL,Ee as BACKFILL_BLOCKS_FTS_SQL,_e as BACKFILL_BLOCK_ALIASES_SQL,be as BACKFILL_BLOCK_TYPES_SQL,J as BLOCKS_FTS_BACKFILL_MARKER_KEY,q as BLOCK_TYPES_BACKFILL_MARKER_KEY,Le as CLEAR_REPROJECT_REF_MARKER_SQL,qe as CLIENT_SCHEMA_STATEMENTS,Je as CLIENT_SCHEMA_TRIGGER_NAMES,z as CREATE_BLOCKS_ALIAS_DELETE_TRIGGER_SQL,L as CREATE_BLOCKS_ALIAS_INSERT_TRIGGER_SQL,R as CREATE_BLOCKS_ALIAS_UPDATE_TRIGGER_SQL,O as CREATE_BLOCKS_DELETE_ROW_EVENT_TRIGGER_SQL,G as CREATE_BLOCKS_FTS_DELETE_TRIGGER_SQL,pe as CREATE_BLOCKS_FTS_INSERT_TRIGGER_SQL,p as CREATE_BLOCKS_FTS_ROWIDS_TABLE_SQL,m as CREATE_BLOCKS_FTS_TABLE_SQL,me as CREATE_BLOCKS_FTS_UPDATE_TRIGGER_SQL,E as CREATE_BLOCKS_INSERT_ROW_EVENT_TRIGGER_SQL,F as CREATE_BLOCKS_PARENT_NOT_DELETED_INSERT_TRIGGER_SQL,I as CREATE_BLOCKS_PARENT_NOT_DELETED_UPDATE_TRIGGER_SQL,b as CREATE_BLOCKS_SYNCED_CHANGES_DELETE_TRIGGER_SQL,v as CREATE_BLOCKS_SYNCED_CHANGES_ID_OP_INDEX_SQL,y as CREATE_BLOCKS_SYNCED_CHANGES_INSERT_TRIGGER_SQL,_ as CREATE_BLOCKS_SYNCED_CHANGES_TABLE_SQL,U as CREATE_BLOCKS_TYPE_DELETE_TRIGGER_SQL,V as CREATE_BLOCKS_TYPE_INSERT_TRIGGER_SQL,H as CREATE_BLOCKS_TYPE_UPDATE_TRIGGER_SQL,D as CREATE_BLOCKS_UPDATE_ROW_EVENT_TRIGGER_SQL,j as CREATE_BLOCKS_UPLOAD_INSERT_TRIGGER_SQL,M as CREATE_BLOCKS_UPLOAD_UPDATE_TRIGGER_SQL,N as CREATE_BLOCKS_WORKSPACE_INVARIANT_INSERT_TRIGGER_SQL,P as CREATE_BLOCKS_WORKSPACE_INVARIANT_UPDATE_TRIGGER_SQL,re as CREATE_BLOCK_ALIASES_TABLE_SQL,B as CREATE_BLOCK_ALIASES_WORKSPACE_UNIQUE_TRIGGER_SQL,ie as CREATE_BLOCK_ALIASES_WS_ALIAS_INDEX_SQL,oe as CREATE_BLOCK_ALIASES_WS_ALIAS_LOWER_INDEX_SQL,d as CREATE_BLOCK_TYPES_TABLE_SQL,f as CREATE_BLOCK_TYPES_TYPE_WORKSPACE_INDEX_SQL,ae as CREATE_CLIENT_SCHEMA_STATE_TABLE_SQL,te as CREATE_COMMAND_EVENTS_CREATED_INDEX_SQL,ee as CREATE_COMMAND_EVENTS_TABLE_SQL,ne as CREATE_COMMAND_EVENTS_WORKSPACE_INDEX_SQL,h as CREATE_PS_CRUD_REJECTED_REJECTED_AT_INDEX_SQL,le as CREATE_PS_CRUD_REJECTED_TABLE_SQL,g as CREATE_PS_CRUD_REJECTED_TX_ID_INDEX_SQL,l as CREATE_ROW_EVENTS_BLOCK_INDEX_SQL,u as CREATE_ROW_EVENTS_CREATED_INDEX_SQL,s as CREATE_ROW_EVENTS_TABLE_SQL,c as CREATE_ROW_EVENTS_TX_INDEX_SQL,a as CREATE_TX_CONTEXT_TABLE_SQL,ce as DROP_ANALYZE_INDEX_SET_MARKERS_SQL,se as DROP_BLOCKS_WORKSPACE_TYPE_INDEX_SQL,Ve as RECONCILE_RESCAN_MARKER_PREFIX,we as RECORD_BLOCKS_FTS_BACKFILL_DONE_SQL,ge as RECORD_BLOCK_ALIASES_BACKFILL_DONE_SQL,ye as RECORD_BLOCK_TYPES_BACKFILL_DONE_SQL,Ue as RECORD_RECONCILE_RESCAN_MARKER_SQL,Ie as RECORD_REPROJECT_REF_MARKER_SQL,Se as RECORD_STAGING_NEEDS_APPLY_SEEDED_SQL,Z as RECORD_UNBOUNDED_ANALYZE_DONE_SQL,Be as RECORD_WORKSPACE_BACKFILL_MARKER_SQL,Pe as REPROJECT_REF_MARKER_PREFIX,ke as RESET_ANALYZE_SAMPLE_LIMIT_SQL,o as SEED_TX_CONTEXT_ROW_SQL,Me as SELECT_BLOCKS_COUNT_SQL,Ce as SELECT_BLOCKS_FTS_BACKFILL_DONE_SQL,he as SELECT_BLOCK_ALIASES_BACKFILL_DONE_SQL,ve as SELECT_BLOCK_TYPES_BACKFILL_DONE_SQL,He as SELECT_RECONCILE_RESCAN_MARKER_SQL,Fe as SELECT_REPROJECT_REF_MARKERS_SQL,xe as SELECT_STAGING_NEEDS_APPLY_SEEDED_SQL,je as SELECT_UNBOUNDED_ANALYZE_DONE_SQL,ze as SELECT_WORKSPACE_BACKFILL_MARKERS_SQL,Y as STAGING_NEEDS_APPLY_SEEDED_MARKER_KEY,X as UNBOUNDED_ANALYZE_MARKER_KEY,Re as WORKSPACE_BACKFILL_MARKER_PREFIX,Ye as backfillBlockAliasesIfEmpty,Xe as backfillBlockTypesIfEmpty,Ze as backfillBlocksFtsIfEmpty,Qe as ensureBlockUserUpdatedAtColumn,$e as ensureStagingNeedsApplyColumn,et as ensureUndoGroupIdColumns,tt as getBlocksCount,at as runAnalyzeIfStale,ct as runAnalyzeNow,ot as runSampledStatsRepair};
