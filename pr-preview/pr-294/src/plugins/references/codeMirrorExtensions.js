...
import { EditorSelection, EditorState, Prec } from "../../../node_modules/@codemirror/state/dist/index.js";
...
import { loadRecentBlockIds } from "../quick-find/recents.js";
...
//#region src/plugins/references/codeMirrorExtensions.ts
var referenceAutocompleteTheme = EditorView.theme({
...
*/
var insertAtCaretForMisplacedDiff = Prec.highest(EditorView.inputHandler.of((view, from, to, insert) => {
...
var buildWikilinkSource = ({ repo }) => backlinkCompletionSource({ getAliases: async (filter) => {
	const workspaceId = repo.activeWorkspaceId;
	if (!workspaceId) {
		console.warn("No active workspace for alias search");
		return [];
	}
	const aliases = await searchAliasLabels(repo, {
		workspaceId,
		query: filter,
		recentBlockIds: await loadRecentBlockIds(repo, workspaceId)
	});
	...
	const dateLabels = new Set(dateCompletions.flatMap((candidate) => [candidate.label, candidate.iso]));
	return [...dateCompletions, ...aliases.filter((alias) => !dateLabels.has(alias))];
} });
var buildBlockrefSource = ({ repo }) => blockrefCompletionSource({ searchBlocks: async (filter) => {
	const workspaceId = repo.activeWorkspaceId;
	if (!workspaceId) return [];
	const query = filter.trim();
	return (query ? await repo.query.searchByContent({
		workspaceId,
		query,
		limit: 12
	}).load() : await repo.query.recentBlocks({
		workspaceId,
		limit: 12
	}).load()).map((block) => ({
		id: block.id,
...
var referencesCodeMirrorExtensions = (ctx) => {
	...
	return [
		...
		EditorState.languageData.of(() => [{ autocomplete: wikilinkSource }, { autocomplete: blockrefSource }])
	];
};
//#endregion
export { referencesCodeMirrorExtensions };
