import { AbstractPowerSyncDatabase, Mutex, SqliteBucketStorage, isDBAdapter, isSQLOpenFactory } from "../../../../common/dist/bundle.js";
...
//#region node_modules/@powersync/web/lib/src/db/PowerSyncDatabase.js
var DEFAULT_POWERSYNC_FLAGS = {
	...DEFAULT_WEB_SQL_FLAGS,
	externallyUnload: false
};
var resolveWebPowerSyncFlags = (flags) => {
	return {
...
function assertValidDatabaseOptions(options) {
	if ("database" in options && "encryptionKey" in options) {
		...
		if (isSQLOpenFactory(database) || isDBAdapter(database)) throw new Error(`Invalid configuration: 'encryptionKey' should only be included inside the database object when using a custom ${isSQLOpenFactory(database) ? "WASQLiteOpenFactory" : "WASQLiteDBAdapter"} constructor.`);
	}
}
/**
...
* ```
*/
var PowerSyncDatabase = class PowerSyncDatabase extends AbstractPowerSyncDatabase {
	options;
 ...
 /**
		...
		*/
		...
		if (typeof this.database.getConfiguration == "function") {
			if (this.database.getConfiguration().requiresPersistentTriggers) this.triggersImpl.updateDefaults({ useStorageByDefault: true });
		}
	}
	generateTriggerManagerConfig() {
		return { claimManager: NAVIGATOR_TRIGGER_CLAIM_MANAGER };
	}
	...
	*/
	close(options) {
		return super.close({ disconnect: options?.disconnect ?? !this.resolvedFlags.enableMultiTabs });
	}
	async loadVersion() {
	...
	async runExclusive(cb) {
		if (this.resolvedFlags.ssrMode) return PowerSyncDatabase.SHARED_MUTEX.runExclusive(cb);
	...
	generateSyncStreamImplementation(connector, options) {
		...
		switch (true) {
			case this.resolvedFlags.ssrMode: return new SSRStreamingSyncImplementation(syncOptions);
          ...
          `;
			...
			default: return new WebStreamingSyncImplementation(syncOptions);
		}
	}
};
//#endregion
export { DEFAULT_POWERSYNC_FLAGS, PowerSyncDatabase, resolveWebPowerSyncFlags };
