 ...
 *   - esm.sh imports (an extension may import from there): cache-first in a
 *     single shared, un-namespaced cache — those URLs carry a version, so
 *     they're immutable across generations and need not be re-fetched on every
 *     deploy. Filled lazily on first use; nothing the app itself boots on lives
 *     there.
