...
import { onSyncSettled } from '@/data/internals/firstSync'
...
const initRepo = memoize(
  async (user: User, useRemoteSync: boolean): Promise<Repo> => {
    ...
    const backfillCompletionClaim = createGraphBackfillClaim({
      ...
      get claimantId() { return requireRepo().activeLayoutSessionId },
      tx: (fn, opts) => requireRepo().tx(fn, opts),
      // Mirrors the backfillSyncGate choice below: a local-only session has no
      // server to be behind, so waiting for `connected && !downloading` would
      // hang the claim forever.
      syncSettled: useRemoteSync
        ? (cb) => onSyncSettled(db as unknown as Parameters<typeof onSyncSettled>[0], cb)
        : (cb) => { cb(); return () => {} },
