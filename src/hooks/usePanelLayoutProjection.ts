  ...
  useEffect(() => {
    ...
    void projection.start()
      .then(async () => {
        ...
        // — so a projection constructed AFTER boot (e.g. an external host
        // switching to an already-materialized session behind a freshly
        // pushed slot-less `#ws;persp=…` hash) would otherwise never
        // normalize its session's rows into the hash, and its first row
        // mutation would push a spurious extra history entry over the stale
        // one. For the BOOT session this re-applies what bootstrapWorkspace
        // just applied and canonicalized — a provable no-op (topology-equal
        // reconcile writes
