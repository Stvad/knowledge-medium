...
// Must match `openExtensionsSettingsAction.id`. Referenced by id rather
// than by importing the action so this generic module stays off the
// extensions-settings plugin's transitive import graph (navigation, prefs
// blocks); `runActionById` is a string-keyed registry by design.
const OPEN_EXTENSIONS_SETTINGS_ACTION_ID = 'open_extensions_settings'

export const toastExtensionLoadError = (
  seen: Set<string>,
  key: string,
  blockId: string,
  error: Error,
...
const openExtensionsSettings = (): void => {
  try {
    void Promise.resolve(
      runActionById(
        OPEN_EXTENSIONS_SETTINGS_ACTION_ID,
        new CustomEvent('extension-load-error'),
      ),
    ).catch((error) =>
      console.error('Failed to open Extensions settings from load-error toast', error),
    )
  } catch (error) {
    // runActionById throws if <HotkeyReconciler/> isn't mounted; by the time
    // a toast is on screen it is, but stay defensive — a failed jump
