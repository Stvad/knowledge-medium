...
let current: LocalDatabaseCorruptError | null = null
const subscribers = new CallbackSet('localDbCorruption')
 ...
 */
export const reportRuntimeLocalDbCorruption = (userId: string, cause: unknown): void => {
  if (current) return
...
export const getLocalDbCorruptionSnapshot = (): LocalDatabaseCorruptError | null => current

/** `useSyncExternalStore` subscribe. */
export const subscribeLocalDbCorruption = (callback: () => void): (() => void) =>
  subscribers.add(callback)

/** Test-only: clear the latch between cases. */
export const __resetLocalDbCorruptionSignalForTest = (): void => {
  current = null
}
