...
const SAFE_VIA_PROTOTYPE: Record<string, string> = {
  ...
  setLayoutSessionRouter: 'interior mutation of the shared ClientContext (router slot) — assigns no Repo field, safe through the chain',
...
const SAFE_INSTANCE_FIELDS: Record<string, string> = {
  graphMigrationLocked: 'arrow field bound to the REAL repo at construction, so reaching it through the facade still reads the real `this` — which it uses for nothing but constants: the workspace, the db handle and the prior-row lookup all arrive as arguments. Handed to `runTx` per call and not stored anywhere that outlives it',
