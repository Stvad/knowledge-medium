...
describe('applyPropertyDefinitionSynthesis: the id is occupied, or the key stopped being orphaned', () => {
  ...
  it('leaves the history alone on a run that mints nothing', async () => {
    // Clearing is a real cost to the user, owed only where there is a hazard.
    // A run with no candidates wrote nothing for a replay to land on top of.
    await rawCell('b1', {})
    await getOrCreatePropertiesPage(repo, WS)
    await repo.tx(async tx => { await tx.update('b1', {content: 'user edit'}) },
      {scope: ChangeScope.BlockDefault, description: 'user edit'})
  ...
  it('refuses a replay queued behind the minting transaction', async () => {
    // Drives the scenario end to end: a replay waits on the write lock the
    // minting transaction holds, and must not put a pre-synthesis snapshot back
    // over what that transaction committed.
    //
    // It does NOT pin WHERE the invalidation happens. The window that needs the
    // in-lock position is between the database handing the lock to this replay
    // and `repo.tx` resolving, and the harness cannot schedule into it — the
    // clear after the await passes this too. Kept as the end-to-end statement,
    // and it would catch a regression that stopped refusing at all.
    await rawCell('b1', {'demo:orphan': 'hello'})
    await getOrCreatePropertiesPage(repo, WS)
    await repo.tx(async tx => { await tx.update('b1', {content: 'user edit'}) },
      {scope: ChangeScope.BlockDefault, description: 'user edit'})
    ...
    // The in-tx gap probe is a fence on "the write lock is held right now": the
    // pass re-asks it inside the transaction, after the one it asks before.
    let releaseTx: (() => void) | null = null
    let announceTx: (() => void) | null = null
    const txOpen = new Promise<void>(resolve => { announceTx = resolve })
    let probeCalls = 0
    const realGap = repo.workspaceViewGap.bind(repo)
    vi.spyOn(repo, 'workspaceViewGap').mockImplementation(async (ws: string) => {
      const gap = await realGap(ws)
      if (announceTx === null) return gap
      // The SECOND call is the in-transaction one.
      if (probeCalls++ === 0) return gap
      announceTx()
      announceTx = null
      await new Promise<void>(resolve => { releaseTx = () => resolve() })
      return gap
    })

    const running = applyPropertyDefinitionSynthesis(repo, plan)
    await txOpen
    ...
    const row = await repo.db.get<{content: string}>(
      'SELECT content FROM blocks WHERE id = ?', ['b1'])
