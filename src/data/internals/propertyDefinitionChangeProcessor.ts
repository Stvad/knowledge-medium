 ...
 * `consumingParentIds`, which is its gate. A consumer that has none is
 * invisible to that gate, which is why nothing here guards the window while the
 * cell-to-children backfill runs: the commit pipeline's migration lock refuses
 * the whole transaction before this processor is reached (#1057).
 */
