...
describe('importRoam', () => {
  ...
  it('writes only into the workspaceId option, never bleeds into others', async () => {
    ...
    // Cross-workspace check: imported root/alias rows carry
    // workspace_id = WORKSPACE. OTHER_WS still has just its seed root
    // row; child-backed property rows under that seed are expected.
    const wsCounts = await env.h.db.getAll<{workspace_id: string; n: number}>(
      'SELECT workspace_id, COUNT(*) AS n FROM blocks WHERE deleted = 0 AND parent_id IS NULL GROUP BY workspace_id ORDER BY workspace_id',
    )
    const byWs = new Map(wsCounts.map(r => [r.workspace_id, r.n]))
    expect(byWs.get(OTHER_WS)).toBe(1) // only the pre-seed root
