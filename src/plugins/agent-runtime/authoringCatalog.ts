...
const guides: AuthoringGuide[] = [
  ...
  },
  {
    ...
    principles: [
      'Two equally-valid shapes: (a) appMountsFacet + window CustomEvent for a persistent mount that can react to live state; (b) imperative `openDialog(Component, props)` from an action handler when you just need a one-shot prompt that returns a value.',
    ...
    steps: [
      'Pick the shape: (a) appMountsFacet + window CustomEvent if the dialog will live across the session and respond to events; (b) `openDialog(Component)` from the action handler when "open it, get a value, close it" is all you need.',
