...
const USER = 'u1'
const WS = 'ws1'
// The canonical staged plaintext + its REAL hash: the drain verifies the local bytes
// against the record's hash before sealing, so a placeholder hash would quarantine.
const PLAIN = new Uint8Array(Array.from({ length: 32 }, (_, i) => i & 0xff)) as Uint8Array<ArrayBuffer>
...
describe('drainUploads (Phase 5b — the up-lane)', () => {
  ...
  it('uploads a pending plaintext asset and deletes the record on success', async () => {
    ...
    expect([...blobStore.puts[0].bytes]).toEqual([...PLAIN])
  ...
  it('encode-at-drain SEALS e2ee bytes — the upload is ciphertext that round-trips', async () => {
    ...
    await store.stage(stageInput())
    await store.promote(USER, BLOCK)
    const plain = PLAIN // the staged record's hash is PLAIN's — the drain verifies before sealing
    await byteStore.put(USER, WS, KEY, plain)
  ...
  it('refuses to upload local bytes that do not hash to the record (a partial / foreign entry): deletes them, quarantines', async () => {
    await store.stage(stageInput())
    await store.promote(USER, BLOCK)
    await byteStore.put(USER, WS, KEY, PLAIN.slice(0, 20)) // a write cut short

    const summary = await drainUploads(USER, deps())

    expect(summary).toMatchObject({ failed: 1, uploaded: 0 })
    expect(blobStore.puts).toHaveLength(0) // never sealed, never sent
    expect(await byteStore.has(USER, WS, KEY)).toBe(false) // the bad entry is gone
    expect((await store.get(USER, BLOCK))?.status).toBe('failed')
  })

  it('refuses an EMPTY local entry the same way', async () => {
    await store.stage(stageInput())
    await store.promote(USER, BLOCK)
    await byteStore.put(USER, WS, KEY, new Uint8Array(0))

    expect(await drainUploads(USER, deps())).toMatchObject({ failed: 1, uploaded: 0 })
    expect(blobStore.puts).toHaveLength(0)
  })
