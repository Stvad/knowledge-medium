...
export interface DownLaneSummary {
  ...
  /** Freshly fetched + verified + stored this pass — the only outcome that consumes budget. */
  readonly replicated: number
  /** A FETCH was attempted but failed (offline / missing / poisoned). Free + re-tried next pass. */
  readonly failed: number
  /** Can't replicate now WITHOUT a fetch (locked / no key / malformed hash). Free. */
  readonly unavailable: number
  /** Left unattempted because the per-pass success budget was reached — the lazy tail. */
...
export const reconcileDownLane = async (
  ...
  for (let i = 0; i < requests.length; i++) {
    ...
    if (PRE_FETCH_FAIL_REASONS.has(r.reason)) {
      unavailable += 1 // locked / no key / malformed — no fetch at all
    } else {
      failed += 1 // a fetch was attempted and failed (offline / missing / poisoned)
    }
  }
