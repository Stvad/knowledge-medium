 ...
 * {@link RETRY_UPLOADS_ACTION_ID} action so the chip renders a "Retry" button — the
 * §9 explicit-user-retry trigger, which force-runs recovery over the failed records.
 */
      ...
      cachedSnapshot =
        failedCount > 0
          ? {
              ...
              // The §9 explicit-user-retry trigger: force-run recovery over the failed
              // records (bypasses the automatic per-record re-drive bound — the user asked).
