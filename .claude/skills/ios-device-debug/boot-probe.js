...
(async () => {
  ...
  // This user's ui-state root for the workspace in the URL (`#<workspaceId>/…`);
  // a shared DB holds one per collaborator per workspace, so neither recency nor
  // content alone identifies the active one.
  const uid = p.getActiveUserId()
  const wsFromHash = (location.hash.match(/^#([0-9a-f-]{36})/i) || [])[1]
  const ui = await db.getAll(
    `select id, workspace_id from blocks where content = 'ui-state' and deleted = 0 and created_by = ?` +
    (wsFromHash ? ` and workspace_id = ?` : ``) + ` order by user_updated_at desc limit 1`,
    wsFromHash ? [uid, wsFromHash] : [uid])
