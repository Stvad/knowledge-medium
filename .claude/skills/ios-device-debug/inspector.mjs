// WebKit Remote Inspector client for a live iPhone/iPad via ios-webkit-debug-proxy.
// iOS uses the MULTI-TARGET protocol: the JS context lives in a Target, so every
// command is wrapped in Target.sendMessageToTarget and replies arrive as
// Target.dispatchMessageFromTarget. Shared by ios.mjs and boot-trace.mjs.
...
export async function findPage() {
  ...
  if (!dev) throw new Error('no device (is the proxy up / device connected?)')
...
// Returns the raw result object ({value} or {objectId, …}); exits on a throw.
export async function evalIn(s, expression) {
  const ev = (await s.send('Runtime.evaluate', { expression: `Promise.resolve((${expression}))`, returnByValue: false, includeCommandLineAPI: true })).result || {}
...
// Connect to the matching page and enable the Runtime domain.
export async function connect() {
  const { pages, page } = await findPage()
  if (!page) { console.error(`no tab matching "${MATCH}" (${pages.length} tabs). Foreground the app tab on the device.`); process.exit(1) }
