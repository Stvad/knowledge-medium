...
// token registration (typical after a `pnpm agent reload` or after
...
export const createBridgeClient = (options: BridgeClientOptions = {}): BridgeClient => {
  ...
  const requireToken = async (): Promise<string> => {
    ...
    if (!token) {
      throw new Error(
        `No agent token configured for profile "${profileName}". Run \`pnpm agent --profile ${profileName} connect\` to pair the CLI with the app.`,
      )
    }
    return token
  }
