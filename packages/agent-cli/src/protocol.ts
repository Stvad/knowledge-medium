...
export const knownCommandRegistry: Record<KnownCommandType, KnownCommandMeta> = {
  ...
  'install-extension': {
    ...
    description: 'Install a JS extension. Reload is automatic; --verify reports the contributed facets/actions; label defaults to the filename without ext. REFUSES when the install would change which codec a value preset id resolves to here — a core building a different codec than the one live under that id, or an update dropping an id it currently registers (the id then falls back to whatever was underneath) — that re-types every value stored under it with no row edit to migrate from, so nothing downstream repairs it; the refusal names what moved and the definitions and cell counts at stake. That check EXECUTES the source, so it runs only when this install makes the source live — the block passes BOTH gates here, approved AND enabled — or when --verify asks for it. An install onto a block that is new, not approved here, or DISABLED stores source nothing runs and is not checked: pass --verify to check one anyway, and note that enabling it later is not covered. --allow-preset-change installs anyway and reports what it changed.',
    readOnly: false,
  },
  'enable-extension': {
