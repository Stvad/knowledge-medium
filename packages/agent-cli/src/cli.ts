...
const reloadAppAndWait = async ({timeoutMs = 30_000} = {}) => {
  ...
  if (!token) {
    throw new Error(`No agent token configured for profile "${selectedProfileName}". Run \`pnpm agent --profile ${selectedProfileName} connect\` first.`)
  }
...
const connectWithToken = async (
  ...
  try {
    ...
    process.stdout.write(
      ...
      `Make sure the app tab is open. Run \`pnpm agent whoami\` to verify.\n`,
    )
  }
}
  ...
  if (!force) {
    ...
    if (existing) {
      process.stdout.write(
        ...
        `Open or focus the app tab, or run \`pnpm agent whoami\` to recheck. Re-pairing anyway…\n\n`,
      )
    }
  }
  ...
  .action(async () => {
    ...
    if (!token) {
      throw new Error(
        ...
        + `Run \`pnpm agent --profile ${selectedProfileName} connect\` first.`,
      )
    }
...
cli
  ...
  .action(async (name: unknown, options: {workspace?: string, limit?: string}) => {
    ...
    if (!text) throw new Error('page requires a <name> (e.g. `pnpm agent page "Project Alpha"`)')
    await runAndPrint({
      type: 'page',
      name: text,
      ...(options.workspace ? {workspaceId: options.workspace} : {}),
...
cli
  ...
  .action(async (date: unknown, options: {workspace?: string}) => {
    ...
    if (!text) throw new Error('daily-note requires a <date> (e.g. `pnpm agent daily-note yesterday`)')
    await runAndPrint({
      type: 'daily-note',
      date: text,
      ...(options.workspace ? {workspaceId: options.workspace} : {}),
