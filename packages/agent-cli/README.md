...
| `kmagent audit-properties [--workspace <id>]` | Every property key in the workspace's data the registry does not resolve — the keys property migration skips silently. Per key: cells, sample blocks, the types carrying it, why it doesn't resolve, and the fix. |
