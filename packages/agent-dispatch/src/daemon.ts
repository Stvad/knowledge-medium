#!/usr/bin/env node
/**
 * km-agent-dispatch — polls the live client through the kmagent bridge,
 * turns backlink/query changes into local agent CLI runs, and threads
 * replies back into the graph.
 *
 * Requires: an open app tab paired with the bridge (kmagent connect,
 * dedicated profile) and a login-authenticated executor CLI.
 * Usage: km-agent-dispatch [--config <path>] [--once]
 *
...
import { BLOCKED_WIKILINKS_ENV, encodeBlockedWikilinks, MCP_SERVER_NAME } from '@knowledge-medium/agent-cli/mcpShared'
...
/** The generic km MCP server definition, shared between claude's
...
const buildMcpServerDef = (config: DaemonConfig) => {
  const mcpServerScript = fileURLToPath(import.meta.resolve('@knowledge-medium/agent-cli/mcp'))
...
/** Generated --mcp-config for spawned claude runs: the generic km
 *  server, bound to the daemon's own profile, with watcher targets
 *  blocked from write-back (the MCP server resolves each name to its
 *  page's full alias set + id itself). */
const writeMcpConfig = async (config: DaemonConfig): Promise<string> => {
  ...
  const configPath = path.join(agentRuntimeConfigDir(), 'agent-dispatch-mcp.json')
