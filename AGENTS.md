...
- a gesture with an EDIT-MODE branch has TWO write paths, and only one is a tx. The CodeMirror branch dispatches into the live doc and the editor's own debounced commit persists it later — so a guard inside your tx is too late and cannot undo it. Refuse BEFORE dispatching, with `blockContentIsOpaque(block)` (`@/data/block`). Ask-agent, insert-image and insert-timestamp each shipped with the tx half guarded and the editor half open.
- refuse before any EXPENSIVE or side-effecting step, not merely before the write. Insert-image refused after `captureFilesToReferences`, which mints asset blocks and starts uploads — so the refusal stranded that media unattached and invisible. The first trustworthy moment after a user-length pause is where the check belongs.
- a filter added downstream of a CACHED, LIMIT-ed query is never the whole change. It needs all three, every time: the filter, an invalidation dep (`typedBlocks.type` per opaque type — gaining a type is a property-only edit that `kernel.content` does not report), and an OVER-FETCH, because the filter runs after the SQL limit and an opaque run at the top of the window otherwise returns nothing. Three review rounds went to re-deriving this for a second call site. Widen relative to the fetch, never to a fixed ceiling — `fetchLimit` floors at `limit`, so a ceiling test is dead for large limits.
...
- when the test needs a row to be CROWDED OUT of a bounded window, make the ordering deterministic by rank, not by recency: a fast test stamps every row in the same millisecond, so a `user_updated_at DESC` tiebreak puts whatever you created last on top — including the row you meant to bury. Two starvation tests here passed with the guard removed for exactly this reason. Use the id, or a lower match bucket, whichever the query actually orders by.
