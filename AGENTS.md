...
- **memory lives in beads** (migrated 2026-08-14): all 95 memory files are stored as `bd remember` entries keyed by their slug, verified recoverable, and synced to the private remote — so memory now survives a machine and is visible to Codex too. Write new memories with `bd remember --key <slug> -- <content>`; recall with `bd memories <keyword>` / `bd recall <key>`. Three mechanics that are easy to get wrong:
  ...
  - **authority model: the `.md` files are the WRITE surface, beads is the durable synced copy.** The harness auto-loads `MEMORY.md` and its own instructions say to write memories as files, so files stay where new memories are authored; beads is what makes them survive a machine and reach Codex. They are kept in sync by re-storing the changed file with `bd remember --key <slug> -- <content>` (and `00-index` for `MEMORY.md`). **This sync is manual today — if it is skipped, the beads copy silently rots**, which is exactly the failure mode that let a stale "powersync deploy still needed" claim survive. Files under `~/.claude/projects/<project>/memory/` are untracked and unpushed, so they are the only other copy; don't delete them.
  - oversized or superseded memories are archived verbatim under `memory/archive/` — a subdirectory, so the top-level `*.md` glob that feeds both the harness and the beads sync skips it.
