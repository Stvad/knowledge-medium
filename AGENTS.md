...
- **GitHub issues stay the tracker of record, and beads mirrors them.** All 193 issues (129 open) were imported with `bd github sync --pull-only`; each bead keeps an `External:` link to its GitHub issue, and type/priority/labels carry over. Keep filing and labelling in GitHub. Re-sync with `GITHUB_TOKEN=$(gh auth token) bd github sync --pull-only`. **Never run a bare `bd github sync`** — it is bidirectional by default and would push local beads back out as new public issues.
- **the memory system stays where it is, for now.** The blocks say "do NOT use MEMORY.md files". Claude Code's per-project memory dir is genuinely local-only (`~/.claude` is a git repo with no remote, and its allowlist excludes `projects/`, so nothing there is even committed) — so `bd remember` + the private Dolt remote below WOULD be more durable, and Codex would see it too. Undecided as of 2026-08-14; don't split memory across both stores unilaterally.
