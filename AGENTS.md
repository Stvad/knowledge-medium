...
- **in a CLOUD session the tracker looks empty and is not.** A fresh container has no `bd` binary and no data, and the FIRST `bd` command auto-creates an empty local DB — which then refuses to pull ("histories have diverged"). Do not conclude the tracker is empty: it has ~199 issues. Sequence: install `bd` (`curl -fsSL https://raw.githubusercontent.com/steveyegge/beads/main/scripts/install.sh | bash`, then add `/root/go/bin` to PATH), attach `Stvad/knowledge-medium-beads` to the session (it is outside the default GitHub scope), move the auto-created `.beads/embeddeddolt` aside, and `bd bootstrap`.
- **cloud sessions cannot WRITE beads at all today, for two independent reasons.** (1) The shared DB sits at schema v65 from an accidental bd v1.2.0/v1.2.1 release while the supported binary knows v53, so every issue mutation dies on a missing `events` table — reads need `BD_IGNORE_SCHEMA_SKEW=1`, and `bd remember`/`forget` happen to work (different write path). (2) `bd dolt push` gets HTTP 403: the cloud proxy attaches its scoped GitHub credential only to traffic it recognizes as git smart-HTTP, and dolt's table-file upload is a different shape. Additive `git push` works, so route durable output to the MAIN repo instead — issues to the GitHub tracker, general learnings to this file — and migrate to beads once sync is fixed.
