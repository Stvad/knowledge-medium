...
- **but a push ADDS a duplicate label taxonomy, and that is unresolved.** Pushing one issue created `type::bug` and `priority::medium` as new repo labels and stuck them on the issue, alongside the existing `bug`/`P2`/`ui`. There is no config to disable label pushing. A full bidirectional run would therefore mint ~10 `type::`/`priority::` labels in the PUBLIC repo and add two to all 129 open issues. Until that is settled, push selectively (`--push-only --issues=<bead-id>`) and expect to clean up labels, or stay on `--pull-only`.
- **memory lives in beads** (migrated 2026-08-14): all 95 memory files are stored as `bd remember` entries keyed by their slug, verified recoverable, and synced to the private remote — so memory now survives a machine and is visible to Codex too. Write new memories with `bd remember --key <slug> -- <content>`; recall with `bd memories <keyword>` / `bd recall <key>`. Three mechanics that are easy to get wrong:
  ...
  - `prime.max-memory-chars` drops WHOLE memories **alphabetically**; it does not truncate them. That is the lever: the curated index is keyed `00-index` so it sorts first and always survives the cap (set to 24000). Session start injects the index plus a few more and a pointer to the rest — same read-on-demand shape as before, ~30KB total. **Keep headroom above the index's own size** (~16KB): the cap drops whole memories, so an index that outgrows it would vanish entirely rather than truncate.
  - the source `.md` files under `~/.claude/projects/<project>/memory/` are still present and are **the only other copy** (that dir is untracked — `~/.claude` is a git repo with no remote whose allowlist excludes `projects/`). Do not delete them without Vlad's say-so.
