...
extension trust gate (issue #67 — do not reintroduce auto-approve):
- a user-extension block runs only when BOTH hold: (1) synced INTENT is true (`extensions:overrides`), and (2) THIS device has a device-local APPROVAL record (a `km-extension-compiled` row, presence = trust) whose `sourceHash` was pinned via `approveExtension`. The loader (`dynamicExtensions.ts`) runs the approval's PINNED `compiled` output, NEVER live `block.content` — so a synced source change can't silently execute. On drift it keeps running the pin and reports `update-available`; with no approval it reports `needs-approval`.
- approval rows are written ONLY by `approveExtension` (the only Babel + row-writing path). Do NOT make compiling/loading write a row (that was Phase-1's implicit auto-approve and is the bug #67 fixes). `loadApprovedExtension` only recompiles from `approvedSource` on a compiler-version bump; `compileForVerification` is live-but-non-persisting (agent `--verify` only).
- enable = approve (trust) + intent; disable = intent only (the pin persists for frictionless re-enable + cross-device disable via the intent gate); uninstall = delete block + `revokeExtensionApproval`. Settings checkbox approves only on FIRST enable (drift surfaces as "Update", never silently adopted); the agent `enable-extension` always re-approves the live source (authorized local actor / its update path).
